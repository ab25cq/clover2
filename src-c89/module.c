...
int CLParser_initialize(CLVALUE * * stack_ptr,
                        ...
                        sVMInfo * info);
int load_module_from_file(sCLModule * * self, char * module_name);
sCLModule * gModules[256];
void module_init()
{
    memset(gModules, 0, sizeof(gModules));
}
static int append_module_to_table(char * name, sCLModule * module)
{
    int hash_value;
    sCLModule * * p;
    hash_value = get_hash_key(name, 256);
...
{
    sCLModule * self;
    self = xcalloc(1, sizeof(sCLModule));
    sBuf_init(&self->mBody);
    xstrncpy(self->mName, module_name, 64);
    xstrncpy(self->mSName, sname, 4096);
    self->mSLine = sline;
...
static void remove_module_from_table(char * module_name)
{
    int hash_value;
    sCLModule * * p;
    hash_value = get_hash_key(module_name, 256);
    p = gModules + hash_value;
    while (1)
    {
        if (*p == (void *) 0)
...
static int save_module_to_file(sCLModule * self)
{
    FILE * f;
    char fname[4096];
    snprintf(fname, 4096, "%s.clm", self->mName);
    f = fopen(fname, "w+");
    if (f == (void *) 0)
    {
        return 0;
...
static int search_for_module_file_from_module_name(char * module_file,
                                                   unsigned int module_file_size,
                                                   char * module_name)
{
    int i;
    char * home;
    ...
    if (home)
    {
        snprintf(module_file,
                 module_file_size,
                 "%s/.clover2/%s.clm",
                 home,
                 module_name);
        if (access(module_file, 0) == 0)
        {
            return 1;
        }
    }
...
int load_module_from_file(sCLModule * * self, char * module_name)
{
    ...
    char sline_buf[128];
    char buf[8192 + 1];
    if (!search_for_module_file_from_module_name(fname,
                                                 4096,
                                                 module_name))
    {
        return 0;
