                   ...
                   sCompileInfo * cinfo)
{
    ...
    if (!(method->mFlags & 0x2))
    {
        ...
        for (i = 0; i < info->generics_info.mNumParams; i++)
        {
            ...
            snprintf(class_name, 64, "GenericsParametorClass%d", i);
        ...
        {
            parser_err_msg(info,
                           "overflow the table or a variable which has the same name exists");
            return 0;
        }
    }
    for (i = 0; i < num_params; i++)
    {
        sParserParam * param;
        param = params + i;
        if (!add_variable_to_table(info->lv_table,
                                   param->mName,
                                   param->mType))
        {
            parser_err_msg(info,
    ...
    cinfo2.pinfo = info;
    cinfo2.type = (void *) 0;
    cinfo2.num_break_points = (void *) 0;
    cinfo2.break_points = (void *) 0;
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if (info->err_num == 0)
            {
                append_opecode_to_code(cinfo2.code, 14, cinfo2.no_output);
    ...
    result_type = create_node_type_from_cl_type(method->mResultType,
                                                info->klass);
    if (!(method->mFlags & 0x2) && strcmp((char *) ((&info->klass->mConst)->mConst + method->mNameOffset),
                                          "initialize") == 0)
    {
        arrange_stack(&cinfo2);
        append_opecode_to_code(cinfo2.code, 32, 0);
        append_int_value_to_code(cinfo2.code, 0, 0);
        append_int_value_to_code(cinfo2.code, 4, 0);
        append_opecode_to_code(cinfo2.code, 8, 0);
    }
    else if (type_identify_with_class_name(result_type, "Null"))
    {
        arrange_stack(&cinfo2);
        append_opecode_to_code(cinfo2.code, 43, 0);
        append_opecode_to_code(cinfo2.code, 8, 0);
    }
    else if (gNodes[node].mNodeType == kNodeTypeReturn)
    {
    ...
    else
    {
        ...
        {
            parser_err_msg(info,
                           "Require return value. Stack num is %d",
                           cinfo2.stack_num);
            cinfo2.err_num++;
        }
        append_opecode_to_code(cinfo2.code, 8, 0);
    }
    var_num = get_var_num(cinfo2.lv_table);
