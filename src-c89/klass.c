...
sCLType * create_cl_type(sCLClass * klass, sCLClass * klass2);
void free_cl_type(sCLType * cl_type);
sClassTable * gHeadClassTable = (void *) 0;
static sClassTable gClassTable[512];
unsigned int get_hash_key(char * name, unsigned int max)
{
    unsigned int result;
    char * p;
...
void remove_class(char * class_name)
{
    ...
    sClassTable * p;
    hash_key = get_hash_key(class_name, 512);
    p = gClassTable + hash_key;
    while (1)
    {
        if (p->mName == (void *) 0)
        {
...
static int search_for_class_file(char * class_name,
                                 char * class_file_name,
                                 size_t class_file_name_size)
{
    char * home;
    ...
    if (home)
    {
        snprintf(class_file_name,
                 class_file_name_size,
                 "%s/.clover2/%s.oclcl",
                 home,
                 class_name);
        if (access(class_file_name, 0) == 0)
        {
            return 1;
        }
    }
...
static int read_from_file(int fd, void * buf, size_t size)
{
    size_t size2;
...
static int read_char_from_file(int fd, char * c)
{
    return read_from_file(fd, c, sizeof(char));
}
static int read_int_from_file(int fd, int * n)
{
    return read_from_file(fd, n, sizeof(int));
}
static int read_long_from_file(int fd, long long * n)
{
    return read_from_file(fd, n, sizeof(long long));
}
static int read_const_from_file(int fd,
                                sConst * constant,
                                char * class_name)
{
    int len;
...
static int read_cl_type_from_file(int fd, sCLType * * cl_type);
static int read_cl_block_type_from_file(int fd,
...
static int read_cl_type_from_file(int fd, sCLType * * cl_type)
{
    int n;
    int i;
    *cl_type = xcalloc(1, sizeof(sCLType));
...
static int read_code_from_file(int fd, sByteCode * code)
{
    int n;
    if (!read_int_from_file(fd, &n))
    {
...
static int read_methods_from_file(int fd,
                                  sCLMethod * * methods,
                                  int * num_methods,
                                  int * size_methods,
                                  sCLClass * klass)
{
    ...
    *methods = xcalloc(1, sizeof(sCLMethod) * *num_methods);
    for (i = 0; i < *num_methods; i++)
    {
        ...
        method->mNativeMethod = (void *) 0;
        method->mNativeFunName = (void *) 0;
...
static int read_fields_from_file(int fd,
                                 sCLField * * fields,
                                 int * num_fields,
                                 int * size_fields,
                                 sCLClass * klass)
{
    ...
    *fields = xcalloc(1, sizeof(sCLField) * *num_fields);
...
static sCLClass * read_class_from_file(char * class_name, int fd)
{
    ...
    int i;
    long long l;
    klass = xcalloc(1, sizeof(sCLClass));
    if (!read_int_from_file(fd, &n))
    {
        xfree(klass);
        return (void *) 0;
...
static int create_virtual_method_table(sCLClass * klass)
{
    int i;
    for (i = 0; i < klass->mNumMethods; i++)
    {
        ...
        method_name_and_params = (char *) ((&klass->mConst)->mConst + method->mMethodNameAndParamsOffset);
        hash_key = get_hash_key(method_name_and_params, 256);
        it = klass->mVirtualMethodTable + hash_key;
        while (1)
        {
            if (*it == (void *) 0)
            {
            ...
            {
                it++;
                if (it == klass->mVirtualMethodTable + 256)
                {
                                                        ...
                                                        char * method_name_and_params)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            char * method_name_and_params2;
            method = *it;
            method_name_and_params2 = (char *) ((&klass->mConst)->mConst + method->mMethodNameAndParamsOffset);
...
static int ready_for_typedef(sCLClass * klass)
{
    int i;
    for (i = 0; i < klass->mNumTypedef; i++)
    {
        ...
        char * class_name2;
        sCLClass * klass;
        class_name1 = (char *) ((&klass->mConst)->mConst + klass->mTypedefClassName1Offsets[i]);
        class_name2 = (char *) ((&klass->mConst)->mConst + klass->mTypedefClassName2Offsets[i]);
        klass = get_class_with_load(class_name2);
        if (klass)
        {
                                      ...
                                      char * class_file_name)
{
    ...
    char c;
    sCLClass * klass;
    fd = open(class_file_name, 00);
    if (fd < 0)
    {
        return (void *) 0;
    }
    if (!read_from_file(fd, &c, 1) || c != 11)
    {
        close(fd);
        return (void *) 0;
    }
    if (!read_from_file(fd, &c, 1) || c != 12)
    {
        close(fd);
        return (void *) 0;
    }
    if (!read_from_file(fd, &c, 1) || c != 34)
    {
        close(fd);
        return (void *) 0;
    }
    if (!read_from_file(fd, &c, 1) || c != 55)
    {
        close(fd);
        return (void *) 0;
    }
    if (!read_from_file(fd, &c, 1) || c != 'C')
    {
        close(fd);
        return (void *) 0;
    }
    if (!read_from_file(fd, &c, 1) || c != 'L')
    {
        close(fd);
        return (void *) 0;
    }
    if (!read_from_file(fd, &c, 1) || c != 'O')
    {
        close(fd);
        return (void *) 0;
    }
    if (!read_from_file(fd, &c, 1) || c != 'V')
    {
        close(fd);
        return (void *) 0;
    }
    if (!read_from_file(fd, &c, 1) || c != 'E')
    {
        close(fd);
        return (void *) 0;
    }
    if (!read_from_file(fd, &c, 1) || c != 'R')
    {
        close(fd);
        return (void *) 0;
    }
    klass = read_class_from_file(class_name, fd);
    close(fd);
    if (klass == (void *) 0)
    {
        fprintf(stderr,
                "Clover2 can't load class %s because of class file\n",
                class_name);
        return (void *) 0;
    }
    memset(klass->mVirtualMethodTable, 0, sizeof(sCLMethod *) * 256);
    ...
    klass->mBoxingClass = (void *) 0;
    klass->mUnboxingClass = (void *) 0;
    klass->mDynamicLibrary = (void *) 0;
    klass->mFreeFun = (void *) 0;
    return klass;
}
sCLClass * load_class(char * class_name)
{
    ...
    if (!search_for_class_file(class_name, class_file_name, 4096))
    {
        return (void *) 0;
    }
                       ...
                       int no_free_object)
{
    ...
    int i;
    klass = xcalloc(1, sizeof(sCLClass));
    klass->mFlags |= (primitive_ ? 0x1 : 0) | (interface ? 0x2 : 0) | (dynamic_class ? 0x10 : 0) | (no_free_object ? 0x20 : 0);
    ...
    klass->mClassNameOffset = append_str_to_constant_pool(&klass->mConst,
                                                          class_name,
                                                          0);
    klass->mMethods = xcalloc(1, sizeof(sCLMethod) * 4);
    klass->mSizeMethods = 4;
    klass->mNumMethods = 0;
    klass->mFields = xcalloc(1, sizeof(sCLField) * 4);
    klass->mSizeFields = 4;
    klass->mNumFields = 0;
    klass->mClassFields = xcalloc(1, sizeof(sCLField) * 4);
    ...
    for (i = 0; i < generics_number; i++)
    {
        ...
        {
            klass->mGenericsParamTypeOffsets[i] = append_str_to_constant_pool(&klass->mConst,
                                                                              (char *) ((&type_of_generics_params[i]->mConst)->mConst + type_of_generics_params[i]->mClassNameOffset),
                                                                              0);
        }
        else
        {
    ...
    klass->mFreeFun = (void *) 0;
    klass->mNumTypedef = 0;
    memset(klass->mTypedefClassName1Offsets, 0, sizeof(int) * 64);
    memset(klass->mTypedefClassName2Offsets, 0, sizeof(int) * 64);
    return klass;
}
static void free_class(sCLClass * klass)
{
    ...
    for (i = 0; i < klass->mNumMethods; i++)
    {
        ...
        if (!(method->mFlags & 0x1) && method->mByteCodes.mCodes != (void *) 0)
        {
            sByteCode_free(&method->mByteCodes);
        }
    }
    xfree(klass->mMethods);
...
sCLType * create_cl_type(sCLClass * klass, sCLClass * klass2)
{
    sCLType * cl_type;
    cl_type = xcalloc(1, sizeof(sCLType));
    cl_type->mClassNameOffset = append_str_to_constant_pool(&klass2->mConst,
...
static void set_boxing_and_unboxing_class(char * primitive_class_name,
                                          char * lapper_class_name)
{
    sCLClass * klass;
    sCLClass * klass2;
    klass = get_class(primitive_class_name);
    ;
    klass2 = get_class(lapper_class_name);
    ;
    if (klass)
    {
...
void class_init()
{
    memset(gClassTable, 0, sizeof(sClassTable) * 512);
    alloc_class("int", 1, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("uint", 1, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("byte", 1, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("ubyte", 1, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("short", 1, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("ushort", 1, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("long", 1, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("ulong", 1, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("float", 1, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("double", 1, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("pointer", 1, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("char", 1, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("bool", 1, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("lambda", 0, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("regex", 0, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("Null", 0, -1, -1, 0, (void *) 0, 0, 0, 1);
    alloc_class("WildCard", 0, -1, -1, 0, (void *) 0, 0, 0, 0);
    alloc_class("Anonymous", 0, -1, -1, 0, (void *) 0, 0, 0, 0);
    alloc_class("Self", 0, -1, -1, 0, (void *) 0, 0, 0, 0);
