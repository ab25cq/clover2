...
static int write_code_and_constant_to_file(sByteCode * code,
                                           sConst * constant,
                                           int var_num,
                                           char * fname)
{
    sBuf buf;
    char output_fname[4096];
    char * p;
    char * p2;
    FILE * f;
    sBuf_init(&buf);
    ...
    *p2 = 0;
    xstrncat(output_fname, ".ocl", 4096);
...
void append_cwd_for_path(char * fname, char * fname2)
{
    if (fname[0] != '/')
    {
        ...
        if (cwd)
        {
            xstrncpy(fname2, cwd, 4096);
            xstrncat(fname2, "/", 4096);
            xstrncat(fname2, fname, 4096);
        }
        else
        {
            xstrncpy(fname2, fname, 4096);
        }
    }
    else
    {
...
char gCompilingSourceFileName[4096];
int compile_script(char * fname, char * source)
{
    sParserInfo info;
    char fname2[4096 + 1];
    ...
    if (!write_code_and_constant_to_file(&code,
                                         &constant,
                                         var_num,
                                         fname))
    {
