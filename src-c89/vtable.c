...
sVarTable * clone_var_table(sVarTable * lv_table)
{
    ...
    sVarTable * it;
    sVar * p;
    result = init_var_table();
    it = lv_table;
    p = it->mLocalVariables;
    while (1)
    {
        if (p->mName[0] != 0)
        {
        ...
        p++;
        if (p == lv_table->mLocalVariables + 128)
        {
            break;
        }
    }
                          ...
                          sNodeType * type_)
{
    int hash_value;
    sVar * p;
    hash_value = get_hash_key(name, 128);
    p = table->mLocalVariables + hash_value;
    while (1)
    {
        if (p->mName[0] == 0)
        {
            xstrncpy(p->mName, name, 64);
        ...
        else
        {
            ...
            {
                p++;
                if (p == table->mLocalVariables + 128)
                {
...
static sVar * get_variable_from_this_table_only(sVarTable * table,
                                                char * name)
{
    int hash_value;
    sVar * p;
...
{
    sVar * var;
    var = get_variable_from_this_table_only(table, name);
    if (var != (void *) 0)
    {
        parser_err_msg(info,
                       "Variable (%s) has already_added in this variable table",
                       name);
        info->err_num++;
    }
}
...
sVarTable * init_block_vtable(sVarTable * lv_table)
{
    ...
    else
    {
        ...
        new_table->mParent = (void *) 0;
    }
    return new_table;
}
void set_max_block_var_num(sVarTable * lv_table,
                           sVarTable * old_table)
{
    int lv_num_of_this_block;
    lv_num_of_this_block = lv_table->mVarNum + lv_table->mMaxBlockVarNum;
