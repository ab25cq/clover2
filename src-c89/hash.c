                           ...
                           sCLClass * class_items)
{
    ...
    klass = get_class("Hash");
    method_name_and_params = "initialize(GenericsParametorClass0[],GenericsParametorClass1[],uint)";
    method = search_for_method_from_virtual_method_table(klass,
                                                         method_name_and_params);
    (*stack_ptr)->mObjectValue = hash_object;
    (*stack_ptr)++;
    keys_array = create_array_object(class_keys, num_elements);
    object_data = (sCLObject *) get_object_pointer(keys_array);
