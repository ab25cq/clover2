...
int gInterpreter = 0;
static int expression_substitution(unsigned int * node,
                                   sParserInfo * info);
int parse_type_for_new(sNodeType * * result_type,
                       unsigned int * array_num,
                       sParserInfo * info);
                                                                          ...
                                                                          "..") != 0)
                            {
                                ...
                                if (stat(path2, &stat_) == 0)
                                {
                                    if (stat_.st_mode & 0100)
                                    {
                                        ...
                                        {
                                            size *= 2;
                                            gCommandNames = xrealloc(gCommandNames,
                                                                     sizeof(char *) * size);
                                        }
                                        snprintf(candidate, 4096, "%s", entry->d_name);
                                        gCommandNames[n++] = xstrdup(candidate);
...
static int parse_word_and_slash(char * buf,
                                int buf_size,
                                sParserInfo * info,
                                int print_out_err_msg,
                                int no_skip_lf)
{
    ...
    if ((*__ctype_b_loc())[(int) *info->p] & (unsigned short int) _ISalpha)
    {
        while ((*__ctype_b_loc())[(int) *info->p] & (unsigned short int) _ISalnum || *info->p == '_' || *info->p == '/')
        {
    ...
    *p2 = 0;
    ...
    if (*info->p == 0 && buf[0] == 0)
    {
        ...
        {
            parser_err_msg(info,
                           "require word(alphabet or number). this is the end of source");
        }
        return 0;
    }
    if (buf[0] == 0)
    {
        ...
        {
            parser_err_msg(info,
                           "require word(alphabet or _ or number). this is (%c)",
                           *info->p);
            info->err_num++;
        }
        if (*info->p == '\n')
               ...
               int no_skip_lf)
{
    ...
    if ((*__ctype_b_loc())[(int) *info->p] & (unsigned short int) _ISalpha)
    {
        while ((*__ctype_b_loc())[(int) *info->p] & (unsigned short int) _ISalnum || *info->p == '_')
        {
            if (p2 - buf < buf_size - 1)
            {
                * (p2++) = *info->p;
                                            ...
                                            sParserInfo * info)
{
    ...
    {
        parser_err_msg(info,
                       "expected that next character is %s, but it is %c(%d)",
                       characters,
                       *info->p,
                       *info->p);
        info->err_num++;
...
int expect_next_character(char * characters, sParserInfo * info)
{
    ...
    while (1)
    {
        ...
        {
            parser_err_msg(info,
                           "clover has expected that next characters are '%s', but it arrived at source end",
                           characters);
            info->err_num++;
            return 1;
        }
    ...
    if (err)
    {
        *perr_characters = 0;
        parser_err_msg(info,
                       "Clover has expected that next characters are '%s', but there are some characters(%s) before them",
                       characters,
                       err_characters);
        info->err_num++;
        info->p++;
...
static int parse_simple_lambda_params(unsigned int * node,
                                      sParserInfo * info,
                                      int lambda)
{
    ...
    if (*info->p == '|')
    {
        ...
        if (lambda)
        {
            if (!parse_params_and_entry_to_lvtable(params,
                                                   &num_params,
                                                   info,
                                                   &new_table,
                                                   (void *) 0,
    ...
    node_block = (void *) 0;
    if (!parse_block(&node_block, info, new_table, 1))
    {
        return 0;
    }
    *node = sNodeTree_create_block_object(params,
                                          num_params,
                                          result_type,
                                          node_block,
                                          lambda,
...
static int parse_method_params(int * num_params,
                               unsigned int * params,
                               sParserInfo * info)
{
    *num_params = 0;
...
static int parse_command_method_params(int * num_params,
                                       unsigned int * params,
                                       sParserInfo * info)
{
    *num_params = 0;
    ...
    else if (*info->p != '\000')
    {
        while (1)
        {
            ...
            while (1)
            {
                ...
                else if (*info->p == ' ' || *info->p == '\t' || *info->p == '\n' || *info->p == ';' || *info->p == '\000' || *info->p == '|' || *info->p == '&')
                {
                    break;
                }
                else
            ...
            {
                unsigned int node;
                sNodeType * command_class;
                unsigned int params2[32];
                int num_params2;
                node = 0;
                node = sNodeTree_create_string_value(param.mBuf,
                                                     (void *) 0,
                                                     (void *) 0,
                                                     0,
                                                     info);
                command_class = create_node_type_with_class_name("Command");
                ;
                num_params2 = 1;
                params2[0] = node;
                node = sNodeTree_create_class_method_call(command_class,
                                                          "expandArg",
                                                          params2,
                                                          num_params2,
                                                          info);
...
static int get_number(int minus,
                      unsigned int * node,
                      sParserInfo * info)
{
    const int buf_size;
    ...
    if ((*__ctype_b_loc())[(int) *info->p] & (unsigned short int) _ISdigit)
    {
        while ((*__ctype_b_loc())[(int) *info->p] & (unsigned short int) _ISdigit)
        {
            * (p2++) = *info->p;
            info->p++;
            if (p2 - buf >= buf_size)
        ...
        *p2 = 0;
        skip_spaces_and_lf(info);
        if (*info->p == '.' && (*__ctype_b_loc())[(int) *(info->p + 1)] & (unsigned short int) _ISdigit)
        {
            info->p++;
            * (p2++) = '.';
            if (p2 - buf >= buf_size)
            ...
            *p2 = 0;
            ...
            else if (*info->p == 'D')
            {
                info->p++;
                skip_spaces_and_lf(info);
                *node = sNodeTree_create_cdouble_value(strtod(buf, (void *) 0),
                                                       0,
                                                       0,
                                                       0,
                                                       info);
            ...
            {
                *node = sNodeTree_create_double_value(strtod(buf, (void *) 0),
                                                      0,
                                                      0,
                                                      0,
                                                      info);
...
static int get_hex_number(unsigned int * node, sParserInfo * info)
{
    ...
    *p = 0;
    ...
    else if (*info->p == 'u' && *(info->p + 1) == 'y')
    {
        info->p += 2;
        skip_spaces_and_lf(info);
        *node = sNodeTree_create_ubyte_value((unsigned char) value,
                                             0,
                                             0,
                                             0,
                                             info);
    ...
    else if (*info->p == 'U' && *(info->p + 1) == 'Y')
    {
        info->p += 2;
        skip_spaces_and_lf(info);
        *node = sNodeTree_create_cubyte_value((unsigned char) value,
                                              0,
                                              0,
                                              0,
                                              info);
    ...
    else if (*info->p == 'S')
    {
        info->p++;
        skip_spaces_and_lf(info);
        *node = sNodeTree_create_cshort_value((short) value,
                                              0,
                                              0,
                                              0,
                                              info);
    ...
    else if (*info->p == 'u' && *(info->p + 1) == 's')
    {
        info->p += 2;
        skip_spaces_and_lf(info);
        *node = sNodeTree_create_ushort_value((unsigned short) value,
                                              0,
                                              0,
                                              0,
                                              info);
    ...
    else if (*info->p == 'U' && *(info->p + 1) == 'S')
    {
        info->p += 2;
        skip_spaces_and_lf(info);
        *node = sNodeTree_create_cushort_value((unsigned short) value,
                                               0,
                                               0,
                                               0,
                                               info);
    ...
    else if (*info->p == 'u')
    {
        info->p++;
        skip_spaces_and_lf(info);
        *node = sNodeTree_create_uint_value((unsigned int) value,
                                            0,
                                            0,
                                            0,
                                            info);
    ...
    else if (*info->p == 'U')
    {
        info->p++;
        skip_spaces_and_lf(info);
        *node = sNodeTree_create_cuint_value((unsigned int) value,
                                             0,
                                             0,
                                             0,
                                             info);
...
static int if_expression(unsigned int * node, sParserInfo * info)
{
    ...
    expect_next_character_with_one_forward(")", info);
    expect_next_character_with_one_forward("{", info);
    if_node_block = (void *) 0;
    if (!parse_block(&if_node_block, info, (void *) 0, 0))
    {
        return 0;
    }
    ...
    while (1)
    {
        ...
        {
            expect_next_character_with_one_forward("{", info);
            if (!parse_block(&else_node_block, info, (void *) 0, 0))
            {
                return 0;
            }
            break;
        ...
        else if (strcmp(buf, "elif") == 0)
        {
            ...
            expect_next_character_with_one_forward(")", info);
            expect_next_character_with_one_forward("{", info);
            if (!parse_block(&elif_node_blocks[elif_num], info, (void *) 0, 0))
            {
                return 0;
            }
            elif_num++;
    ...
    *node = sNodeTree_if_expression(expression_node,
                                    if_node_block,
...
static int while_expression(unsigned int * node,
                            sParserInfo * info)
{
    unsigned int expression_node;
    sNodeBlock * while_node_block;
    ...
    expect_next_character_with_one_forward(")", info);
    expect_next_character_with_one_forward("{", info);
    while_node_block = (void *) 0;
    if (!parse_block(&while_node_block, info, (void *) 0, 0))
    {
        return 0;
    }
    *node = sNodeTree_while_expression(expression_node,
                                       while_node_block,
                                       info);
    return 1;
}
static int for_expression(unsigned int * node, sParserInfo * info)
{
    ...
    expect_next_character_with_one_forward(")", info);
    expect_next_character_with_one_forward("{", info);
    for_node_block = (void *) 0;
    if (!parse_block(&for_node_block, info, (void *) 0, 0))
    {
        return 0;
    }
    *node = sNodeTree_for_expression(expression_node,
                                     expression_node2,
                                     expression_node3,
                                     for_node_block,
                                     info);
...
static int when_expression(unsigned int * node, sParserInfo * info)
{
    ...
    else_block = (void *) 0;
    num_when_block = 0;
    memset(when_types, 0, sizeof(sNodeType *) * 64);
    memset(when_types2, 0, sizeof(sNodeType *) * 64);
    while (1)
    {
        if (*info->p == '}')
        {
        ...
        if (*info->p == 'i' && *(info->p + 1) == 's')
        {
            ...
            expect_next_character_with_one_forward("{", info);
            when_block = (void *) 0;
            if (!parse_block(&when_block, info, (void *) 0, 0))
            {
                return 0;
            }
        ...
        else if (*info->p == 'e' && *(info->p + 1) == 'l' && *(info->p + 2) == 's' && *(info->p + 3) == 'e')
        {
            ...
            if (!parse_block(&else_block, info, (void *) 0, 0))
            {
                return 0;
            }
        }
    ...
    *node = sNodeTree_when_expression(expression_node,
                                      value_nodes,
                                      num_values,
                                      when_blocks,
                                      num_when_block,
...
static int return_expression(unsigned int * node,
                             sParserInfo * info)
{
    unsigned int expression_node;
    expression_node = 0;
...
static int new_expression(unsigned int * node, sParserInfo * info)
{
    ...
    *node = sNodeTree_create_new_operator(node_type,
                                          params,
                                          num_params,
                                          array_num,
                                          info);
...
static int throw_expression(unsigned int * node,
                            sParserInfo * info)
{
    unsigned int expression_node;
    expression_node = 0;
...
static int parse_param(sParserParam * param, sParserInfo * info)
{
    ...
    if (*info->p == '=')
    {
        ...
        param->mDefaultValue[p2 - p] = '\000';
    }
    else
    {
        param->mDefaultValue[0] = '\000';
    }
    return 1;
}
int parse_params(sParserParam * params,
                 ...
                 int character_type)
{
    ...
    while (1)
    {
        ...
        {
            parser_err_msg(info,
                           "It is required to ',' or ')' before the source end");
            info->err_num++;
            break;
        }
        else
        {
            parser_err_msg(info,
                           "Unexpected character(%c). It is required to ',' or ')' or '|' character",
                           *info->p);
            if (*info->p == '\n')
            {
                info->sline++;
...
static int try_expression(unsigned int * node, sParserInfo * info)
{
    ...
    int num_params;
    expect_next_character_with_one_forward("{", info);
    try_node_block = (void *) 0;
    if (!parse_block(&try_node_block, info, (void *) 0, 0))
    {
        return 0;
    }
    catch_node_block = (void *) 0;
    num_params = 0;
    params[0].mName[0] = '\000';
    if (info->err_num == 0)
    {
        sVarTable * new_table;
        ...
        {
            parser_err_msg(info,
                           "Require the type of a catch param should be a exception type");
            info->err_num++;
        }
        expect_next_character_with_one_forward("{", info);
        if (!parse_block(&catch_node_block, info, new_table, 0))
        {
            return 0;
        }
    }
    *node = sNodeTree_try_expression(try_node_block,
                                     catch_node_block,
                                     params[0].mName,
                                     info);
    return 1;
...
int parse_type(sNodeType * * result_type, sParserInfo * info)
{
    ...
    *result_type = (void *) 0;
    ...
    {
        if (strcmp(type_name,
                   info->method_generics_info.mParamNames[i]) == 0)
        {
        ...
        if (*info->p == '(')
        {
            ...
            else
            {
                while (1)
                {
                    ...
                    {
                        parser_err_msg(info,
                                       "invalid character in block type name(%c)",
                                       *info->p);
                        break;
                    }
                }
    ...
    else if (*info->p == '<' && *(info->p + 1) != '<' && *(info->p + 1) != '=')
    {
        ...
        while (1)
        {
            if (!parse_type(&(*result_type)->mGenericsTypes[generics_num],
                            info))
            {
                return 0;
            }
            ...
            {
                parser_err_msg(info,
                               "invalid character(%c) in generics types",
                               *info->p);
                info->err_num++;
                break;
            }
    ...
    if (*info->p == '@')
    {
        ...
        if ((*__ctype_b_loc())[(int) *info->p] & (unsigned short int) _ISalpha)
        {
            while ((*__ctype_b_loc())[(int) *info->p] & (unsigned short int) _ISalnum || *info->p == '-' || *info->p == '_' || *info->p == '[' || *info->p == ']')
            {
                info->p++;
            }
            skip_spaces_and_lf(info);
    ...
    if (info->err_num == 0)
    {
        ...
        for (i = 0; i < generics_num; i++)
        {
            ...
            left_type = get_class_with_load_on_compile_time((char *) ((&klass->mConst)->mConst + klass->mGenericsParamTypeOffsets[i]));
            right_type = (*result_type)->mGenericsTypes[i]->mClass;
            solve_method_generics_for_variable_to_class(right_type,
                                                        &right_type2,
                                                        info);
            solve_generics_for_variable_to_class(right_type2,
                                                 &right_type3,
                                                 info);
            if (!check_implemented_methods_for_interface(left_type,
                                                         right_type3))
            {
                parser_err_msg(info,
                               "%s is not implemented %s interface",
                               (char *) ((&right_type2->mConst)->mConst + right_type2->mClassNameOffset),
                               (char *) ((&left_type->mConst)->mConst + left_type->mClassNameOffset));
                info->err_num++;
            }
        }
    }
                       ...
                       sParserInfo * info)
{
    ...
    *array_num = 0;
    ...
    for (i = 0; i < generics_num; i++)
    {
        int generics_paramType_offset;
        sCLClass * left_type;
        sCLClass * right_type;
        sCLClass * right_type2;
        sCLClass * right_type3;
        generics_paramType_offset = klass->mGenericsParamTypeOffsets[i];
        left_type = get_class_with_load_on_compile_time((char *) ((&klass->mConst)->mConst + generics_paramType_offset));
        right_type = (*result_type)->mGenericsTypes[i]->mClass;
        solve_method_generics_for_variable_to_class(right_type,
                                                    &right_type2,
                                                    info);
...
{
    return *info->p == '+' && *(info->p + 1) == '=' || *info->p == '-' && *(info->p + 1) == '=' || *info->p == '*' && *(info->p + 1) == '=' || *info->p == '/' && *(info->p + 1) == '=' || *info->p == '%' && *(info->p + 1) == '=' || *info->p == '<' && *(info->p + 1) == '<' && *(info->p + 2) == '=' || *info->p == '>' && *(info->p + 1) == '>' && *(info->p + 2) == '=' || *info->p == '&' && *(info->p + 1) == '=' || *info->p == '^' && *(info->p + 1) == '=' || *info->p == '|' && *(info->p + 1) == '=';
}
static int assign_operator(unsigned int * node, sParserInfo * info)
{
    if (*info->p == '+' && *(info->p + 1) == '=')
    {
        unsigned int right;
    ...
    else if (*info->p == '<' && *(info->p + 1) == '<' && *(info->p + 2) == '=')
    {
        ...
        *node = sNodeTree_create_operand(kOpLeftShift,
                                         *node,
                                         right,
                                         0,
                                         info);
    ...
    else if (*info->p == '>' && *(info->p + 1) == '>' && *(info->p + 2) == '=')
    {
        ...
        *node = sNodeTree_create_operand(kOpRightShift,
                                         *node,
                                         right,
                                         0,
                                         info);
...
static int postposition_operator(unsigned int * node,
                                 sParserInfo * info,
                                 int * num_method_chains,
                                 unsigned int max_method_chains_node[128])
{
    ...
    while (*info->p)
    {
        ...
        else if (*info->p == '.' && *(info->p + 1) != '.')
        {
            info->p++;
            skip_spaces_and_lf(info);
            if (info->get_type_for_interpreter && *info->p == '\000')
            {
                return 1;
            }
            if ((*__ctype_b_loc())[(int) *info->p] & (unsigned short int) _ISalpha)
            {
                ...
                if (*info->p == '(' || *info->p == '{')
                {
                    ...
                    *node = sNodeTree_create_method_call(*node,
                                                         buf,
                                                         params,
                                                         num_params,
                                                         *num_method_chains,
                ...
                else
                {
                    if (is_assign_operator(info))
                    {
                        unsigned int node2;
                        unsigned int node3;
                        node2 = clone_node(*node);
                        node3 = clone_node(*node);
                    ...
                    else if (*info->p == '=' && *(info->p + 1) != '=')
                    {
                        ...
                        {
                            *node = sNodeTree_create_assign_field(buf,
                                                                  *node,
                                                                  right_node,
                                                                  info);
                        }
        ...
        else if (*info->p == '[')
        {
            ...
            else
            {
                ...
                {
                    unsigned int node2;
                    unsigned int index_node2;
                    node2 = clone_node(*node);
                    index_node2 = clone_node(index_node);
                    *node = sNodeTree_create_load_array_element(*node,
                                                                index_node,
                                                                info);
                    if (!assign_operator(node, info))
                    {
                    ...
                    *node = sNodeTree_create_store_array_element(node2,
                                                                 index_node2,
                                                                 *node,
                                                                 info);
                }
                else if (*info->p == '=' && *(info->p + 1) != '=')
                {
                    ...
                    {
                        *node = sNodeTree_create_store_array_element(*node,
                                                                     index_node,
                                                                     right_node,
                                                                     info);
                    }
        ...
        else if (*info->p == '-' && *(info->p + 1) == '>')
        {
            ...
            if (*info->p == '=' && *(info->p + 1) != '=')
            {
                ...
                {
                    *node = sNodeTree_create_store_value_to_pointer(*node,
                                                                    node_type,
                                                                    right_node,
                                                                    info);
                }
            ...
            {
                *node = sNodeTree_create_load_value_from_pointer(*node,
                                                                 node_type,
                                                                 info);
            }
        }
...
static int parse_block_object(unsigned int * node,
                              sParserInfo * info,
                              int lambda)
{
    sParserParam params[32];
...
static int parse_function(unsigned int * node,
                          sParserInfo * info,
                          int lambda)
{
    char fun_name[64];
    ...
    *node = sNodeTree_create_function(fun_name,
                                      params,
                                      num_params,
                                      result_type,
                                      node_block,
...
static int parse_normal_block(unsigned int * node,
                              sParserInfo * info)
{
    sNodeBlock * node_block;
    node_block = (void *) 0;
    if (!parse_block(&node_block, info, (void *) 0, 0))
    {
        return 0;
    }
...
static int parse_array_value(unsigned int * node,
                             sParserInfo * info)
{
    int num_elements;
    unsigned int array_elements[256];
    num_elements = 0;
    memset(array_elements, 0, sizeof(unsigned int) * 256);
    if (*info->p == ']')
    {
        info->p++;
        skip_spaces_and_lf(info);
    ...
    *node = sNodeTree_create_array_value(num_elements,
                                         array_elements,
                                         info);
    return 1;
}
static int parse_carray_value(unsigned int * node,
                              sParserInfo * info)
{
    int num_elements;
    unsigned int array_elements[256];
    ...
    *node = sNodeTree_create_carray_value(num_elements,
                                          array_elements,
                                          info);
    return 1;
}
static int parse_equalable_carray_value(unsigned int * node,
                                        sParserInfo * info)
{
    int num_elements;
    unsigned int array_elements[256];
    ...
    *node = sNodeTree_create_equalable_carray_value(num_elements,
                                                    array_elements,
                                                    info);
    return 1;
}
static int parse_sortable_carray_value(unsigned int * node,
                                       sParserInfo * info)
{
    int num_elements;
    unsigned int array_elements[256];
    ...
    *node = sNodeTree_create_sortable_carray_value(num_elements,
                                                   array_elements,
                                                   info);
    return 1;
}
static int parse_hash_value(unsigned int * node,
                            sParserInfo * info)
{
    int num_elements;
    unsigned int hash_keys[256 + 1];
    unsigned int hash_items[256 + 1];
    num_elements = 0;
    memset(hash_keys, 0, sizeof(unsigned int) * 256);
    memset(hash_items, 0, sizeof(unsigned int) * 256);
    if (*info->p == '}')
    {
        info->p++;
        skip_spaces_and_lf(info);
    ...
    *node = sNodeTree_create_hash_value(num_elements,
                                        hash_keys,
                                        hash_items,
                                        info);
    return 1;
}
static int parse_list_value(unsigned int * node,
                            sParserInfo * info)
{
    int num_elements;
    unsigned int list_elements[256];
    num_elements = 0;
    memset(list_elements, 0, sizeof(unsigned int) * 256);
    if (*info->p == '}')
    {
        info->p++;
        skip_spaces_and_lf(info);
    ...
    *node = sNodeTree_create_list_value(num_elements,
                                        list_elements,
                                        info);
    return 1;
}
static int parse_equalable_list_value(unsigned int * node,
                                      sParserInfo * info)
{
    int num_elements;
    unsigned int list_elements[256];
    ...
    *node = sNodeTree_create_equalable_list_value(num_elements,
                                                  list_elements,
                                                  info);
    return 1;
}
static int parse_sortable_list_value(unsigned int * node,
                                     sParserInfo * info)
{
    int num_elements;
    unsigned int list_elements[256];
    ...
    *node = sNodeTree_create_sortable_list_value(num_elements,
                                                 list_elements,
                                                 info);
    return 1;
}
static int parse_tuple_value(unsigned int * node,
                             sParserInfo * info)
{
    int num_elements;
    unsigned int tuple_element[256];
    num_elements = 0;
    memset(tuple_element, 0, sizeof(unsigned int) * 256);
    if (*info->p == '}')
    {
        info->p++;
        skip_spaces_and_lf(info);
    ...
    *node = sNodeTree_create_tuple_value(num_elements,
                                         tuple_element,
                                         info);
    return 1;
}
int parse_iniherit(unsigned int * node, sParserInfo * info)
{
    ...
    *node = sNodeTree_create_inherit_call(num_params,
                                          params,
                                          info->klass->mMethodIndexOnCompileTime - 1,
                                          info);
    return 1;
}
...
static int parse_string_expression(unsigned int * string_expressions,
                                   int * string_expression_offsets,
                                   int * num_string_expression,
                                   sBuf * value,
                                   sParserInfo * info)
{
    ...
    info2.exist_block_object_err = 0;
    node = 0;
    if (!expression(&node, &info2))
    {
        xfree(expression_str.mBuf);
        return 0;
    }
...
static int expression_node(unsigned int * node, sParserInfo * info)
{
    ...
    else if (*info->p == '0' && (*__ctype_b_loc())[(int) *(info->p + 1)] & (unsigned short int) _ISdigit)
    {
        info->p++;
    ...
    else if ((*__ctype_b_loc())[(int) *info->p] & (unsigned short int) _ISdigit)
    {
        if (!get_number(0, node, info))
        {
            return 0;
    ...
    {
        sBuf value;
        unsigned int string_expressions[32];
        int string_expression_offsets[32];
        int num_string_expression;
        info->p++;
        sBuf_init(&value);
        memset(string_expressions, 0, sizeof(unsigned int) * 32);
        memset(string_expression_offsets, 0, sizeof(int) * 32);
        ...
        while (1)
        {
            ...
            else if (*info->p == '\\')
            {
                ...
                {
                case '0':
                    sBuf_append_char(&value, '\000');
                    info->p++;
                    break;
                case '{':
                    info->p++;
                    if (!parse_string_expression(string_expressions,
        ...
        skip_spaces_and_lf(info);
        *node = sNodeTree_create_string_value(value.mBuf,
                                              string_expressions,
                                              string_expression_offsets,
                                              num_string_expression,
                                              info);
    ...
    else if ((*info->p == 'B' || *info->p == 'b') && *(info->p + 1) == '"')
    {
        ...
        skip_spaces_and_lf(info);
        *node = sNodeTree_create_buffer_value(value.mBuf,
                                              value.mLen,
                                              string_expressions,
                                              string_expression_offsets,
                                              num_string_expression,
    ...
    else if ((*info->p == 'P' || *info->p == 'p') && *(info->p + 1) == '"')
    {
        ...
        skip_spaces_and_lf(info);
        *node = sNodeTree_create_path_value(value.mBuf,
                                            value.mLen,
                                            string_expressions,
                                            string_expression_offsets,
                                            num_string_expression,
    ...
    else if ((*__ctype_b_loc())[(int) *info->p] & (unsigned short int) _ISalpha || *info->p == '.' && *(info->p + 1) == '/' || *info->p == '.' && *(info->p + 1) == '.' && *(info->p + 2) == '/')
    {
        char buf[64];
        char * p_before;
        int sline_before;
        ...
        if (!parse_word_and_slash(buf, 64, info, 1, 1))
        {
            return 0;
        }
        skip_spaces_and_lf(info);
        including_slash = strstr(buf, "/") != (void *) 0;
        if (strcmp(buf, "if") == 0)
        {
            skip_spaces_and_lf(info);
        ...
        else if ((strcmp(buf, "equalable_list") == 0 || strcmp(buf,
                                                               "elist") == 0) && *info->p == '{')
        {
            skip_spaces_and_lf(info);
            expect_next_character_with_one_forward("{", info);
        ...
        else if ((strcmp(buf, "sortable_list") == 0 || strcmp(buf,
                                                              "slist") == 0) && *info->p == '{')
        {
            skip_spaces_and_lf(info);
            expect_next_character_with_one_forward("{", info);
        ...
        else if ((strcmp(buf, "equalable_array") == 0 || strcmp(buf,
                                                                "earray") == 0) && *info->p == '{')
        {
            skip_spaces_and_lf(info);
            expect_next_character_with_one_forward("{", info);
        ...
        else if ((strcmp(buf, "sortable_array") == 0 || strcmp(buf,
                                                               "sarray") == 0) && *info->p == '{')
        {
            skip_spaces_and_lf(info);
            expect_next_character_with_one_forward("{", info);
        ...
        else if (*info->p == ':')
        {
            ...
            if (*info->p == '=' && *(info->p + 1) != '=')
            {
                ...
                {
                    *node = sNodeTree_create_store_variable(buf,
                                                            node_type,
                                                            right_node,
                                                            info->klass,
                                                            info);
        ...
        {
            if (get_variable_from_table(info->lv_table,
                                        buf) || is_method_param_name(buf))
            {
                unsigned int right_node;
                skip_spaces_and_lf(info);
                info->p++;
        ...
        else
        {
            ...
            if (klass)
            {
                ...
                if (*info->p == '.')
                {
                    ...
                    if (*info->p == '(')
                    {
                        ...
                        *node = sNodeTree_create_class_method_call(klass2,
                                                                   buf,
                                                                   params,
                                                                   num_params,
                                                                   info);
                    ...
                    else
                    {
                        if (is_assign_operator(info))
                        {
                            ...
                            *node = sNodeTree_create_assign_class_field(klass,
                                                                        buf,
                                                                        *node,
                                                                        info);
                        }
            ...
            else if (method_name_existance(global_klass, buf))
            {
                ...
                global_klass_type = alloc_node_type();
                global_klass_type->mClass = global_klass;
                global_klass_type->mNumGenericsTypes = 0;
                *node = sNodeTree_create_class_method_call(global_klass_type,
                                                           buf,
                                                           params,
                                                           num_params,
                                                           info);
            ...
            else if (info->klass && method_name_existance(info->klass,
                                                          buf) && *info->p == '(')
            {
                unsigned int params[32];
                int num_params;
            ...
            else if (get_variable_from_table(info->lv_table,
                                             buf) || is_method_param_name(buf))
            {
                skip_spaces_and_lf(info);
                *node = sNodeTree_create_load_variable(buf, info);
                if (*info->p == '(')
                {
                    ...
                    *node = sNodeTree_create_block_call(*node,
                                                        num_params,
                                                        params,
                                                        info);
                }
            }
            else if (info->klass && field_name_existance(info->klass,
                                                         buf) && *info->p != '(')
            {
                skip_spaces_and_lf(info);
                *node = sNodeTree_create_load_variable("self", info);
            ...
            else if (including_slash || get_variable_index(info->lv_table,
                                                           buf) == -1 && is_command_name(buf) && *info->p != '(')
            {
                unsigned int params[32];
                int num_params;
                sCLClass * command_klass;
                sNodeType * command_klass_type;
                num_params = 0;
                if (!parse_command_method_params(&num_params, params, info))
                {
                    return 0;
                }
                command_klass = get_class("Command");
                ;
                command_klass_type = alloc_node_type();
                command_klass_type->mClass = command_klass;
                command_klass_type->mNumGenericsTypes = 0;
                *node = sNodeTree_create_class_method_call(command_klass_type,
                                                           buf,
                                                           params,
                                                           num_params,
                                                           info);
                ...
                while (1)
                {
                    ...
                    else if (info->next_command_is_to_bool || *info->p == '&' && *(info->p + 1) == '&' || *info->p == '|' && *(info->p + 1) == '|')
                    {
                        ...
                        if (gNodes[*node].mNodeType == kNodeTypeClassMethodCall)
                        {
                            int string_node;
                            int num_params;
                            string_node = sNodeTree_create_string_value(xstrdup("--controlling-terminal"),
                                                                        (void *) 0,
                                                                        (void *) 0,
                                                                        0,
                                                                        info);
            ...
            else if (get_variable_index(info->lv_table,
                                        buf) == -1 && is_command_name(buf) && *info->p == '(')
            {
                unsigned int params[32];
                int num_params;
    ...
    else if (*info->p == '/' && *(info->p + 1) != '*')
    {
        ...
        skip_spaces_and_lf(info);
        *node = sNodeTree_create_regex(regex.mBuf,
                                       global,
                                       ignore_case,
                                       multiline,
                                       extended,
    ...
    {
        parser_err_msg(info,
                       "invalid character (character code %d) (%c)",
                       *info->p,
                       *info->p);
        if (*info->p == '\n')
        {
    ...
    if (!postposition_operator(node,
                               info,
                               &num_method_chains,
                               max_method_chains_node))
    {
        return 0;
    }
...
static int expression_monadic_operator(unsigned int * node,
                                       sParserInfo * info)
{
    while (*info->p)
    {
        ...
        else if (*info->p == '!')
        {
            ...
            *node = sNodeTree_create_operand(kOpLogicalDenial,
                                             *node,
                                             0,
                                             0,
                                             info);
...
static int expression_implements(unsigned int * node,
                                 sParserInfo * info)
{
    if (!expression_monadic_operator(node, info))
    {
...
static int expression_mult_div(unsigned int * node,
                               sParserInfo * info)
{
...
static int expression_add_sub(unsigned int * node,
                              sParserInfo * info)
{
...
static int expression_shift(unsigned int * node,
                            sParserInfo * info)
{
...
static int expression_comparison_operator(unsigned int * node,
                                          sParserInfo * info)
{
    ...
    while (*info->p)
    {
        if (*info->p == '>' && *(info->p + 1) == '=')
        {
            ...
            *node = sNodeTree_create_operand(kOpComparisonGreaterEqual,
                                             *node,
                                             right,
                                             0,
                                             info);
        ...
        else if (*info->p == '<' && *(info->p + 1) == '=')
        {
            ...
            *node = sNodeTree_create_operand(kOpComparisonLesserEqual,
                                             *node,
                                             right,
                                             0,
                                             info);
        ...
        else if (*info->p == '>' && *(info->p + 1) != '>')
        {
            ...
            *node = sNodeTree_create_operand(kOpComparisonGreater,
                                             *node,
                                             right,
                                             0,
                                             info);
        ...
        else if (*info->p == '<' && *(info->p + 1) != '<')
        {
            ...
            *node = sNodeTree_create_operand(kOpComparisonLesser,
                                             *node,
                                             right,
                                             0,
                                             info);
...
static int expression_comparison_equal_operator(unsigned int * node,
                                                sParserInfo * info)
{
    ...
    while (*info->p)
    {
        if (*info->p == '=' && *(info->p + 1) == '=')
        {
            ...
            *node = sNodeTree_create_operand(kOpComparisonEqual,
                                             *node,
                                             right,
                                             0,
                                             info);
        ...
        else if (*info->p == '!' && *(info->p + 1) == '=')
        {
            ...
            *node = sNodeTree_create_operand(kOpComparisonNotEqual,
                                             *node,
                                             right,
                                             0,
                                             info);
...
static int expression_and_and_or_or(unsigned int * node,
                                    sParserInfo * info)
{
