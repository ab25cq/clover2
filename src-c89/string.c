...
CLObject create_string_object(char * str)
{
    ...
    ;
    obj = create_object(string_class, "String");
    cl_value.mLongValue = 0;
    cl_value.mObjectValue = obj;
    push_value_to_global_stack(cl_value);
    char_class = get_class("char");
    ;
    buffer = create_array_object(char_class, wlen + 1);
    buffer_data = (sCLObject *) get_object_pointer(buffer);
    for (i = 0; i < wlen; i++)
    {
        buffer_data->mFields[i].mCharValue = wstr[i];
    }
    buffer_data->mFields[i].mCharValue = '\000';
...
CLObject create_buffer_object(char * buffer, size_t size)
{
    sCLClass * buffer_class;
    CLObject obj;
    sCLObject * object_data;
    buffer_class = get_class("Buffer");
    ;
    obj = create_object(buffer_class, "Buffer");
    object_data = (sCLObject *) get_object_pointer(obj);
    object_data->mFields[0].mPointerValue = xcalloc(1, size);
    memcpy(object_data->mFields[0].mPointerValue, buffer, size);
    object_data->mFields[1].mULongValue = size;
    object_data->mFields[2].mULongValue = size;
    return obj;
}
CLObject create_path_object(char * path)
{
    ...
    CLObject obj;
    sCLObject * object_data;
    path_class = get_class("Path");
    ;
    str_object = create_string_object(path);
    cl_value.mLongValue = 0;
    cl_value.mObjectValue = str_object;
    push_value_to_global_stack(cl_value);
    obj = create_object(path_class, "Path");
    object_data = (sCLObject *) get_object_pointer(obj);
