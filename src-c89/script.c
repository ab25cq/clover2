...
int eval_file(char * fname, int stack_size)
{
    ...
    code_contents = xmalloc(code_len);
    ...
    code_contents2 = xmalloc(code_len2);
    if (fread(code_contents2, 1, code_len2, f) < code_len2)
    {
        fclose(f);
        xfree(code_contents);
        xfree(code_contents2);
        fprintf(stderr, "Clover2 can't read byte code\n");
        return 0;
    }
    sByteCode_init(&code);
    append_value_to_code(&code, code_contents, code_len, 0);
    sConst_init(&constant);
    sConst_append(&constant, code_contents2, code_len2, 0);
    stack = xcalloc(1, sizeof(CLVALUE) * stack_size);
    memset(&info, 0, sizeof(sVMInfo));
    if (!vm(&code, &constant, stack, var_num, (void *) 0, &info))
    {
