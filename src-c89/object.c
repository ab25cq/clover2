...
void object_mark_fun(CLObject self, unsigned char * mark_flg)
{
    ...
    sCLClass * klass;
    int i;
    object = (sCLObject *) get_object_pointer(self);
    klass = object->mClass;
...
int free_object(CLObject self)
{
    ...
    sCLClass * klass;
    object_data = (sCLObject *) get_object_pointer(self);
    klass = object_data->mClass;
    xfree(object_data->mType);
...
{
    unsigned int size;
    size = sizeof(sCLObject) - sizeof(CLVALUE) * 32;
...
CLObject create_object(sCLClass * klass, char * type)
{
    ...
    obj = alloc_heap_mem(size, klass, -1);
    object_data = (sCLObject *) get_object_pointer(obj);
    object_data->mType = xstrdup(type);
    return obj;
}
char * get_class_name_from_cl_type(sCLType * cl_type,
                                   sCLClass * klass)
{
    return (char *) ((&klass->mConst)->mConst + cl_type->mClassNameOffset);
}
                                               ...
                                               sCLClass * klass2)
{
    ...
    result_type1 = get_class_name_from_cl_type(method1->mResultType,
                                               klass1);
    result_type2 = get_class_name_from_cl_type(method2->mResultType,
                                               klass2);
    ...
    for (i = 0; i < method1->mNumParams; i++)
    {
        ...
        param1_type = get_class_name_from_cl_type(param1->mType, klass1);
        param2_type = get_class_name_from_cl_type(param2->mType, klass2);
        if (strcmp(param1_type, "Self") == 0)
        {
            param1_type = (char *) ((&klass2->mConst)->mConst + klass2->mClassNameOffset);
        }
...
static int check_implemented_methods_for_interface_on_runtime(sCLClass * left_class,
                                                              sCLClass * right_class)
{
                                ...
                                sCLClass * interface)
{
    ...
    klass = object_data->mClass;
    return check_implemented_methods_for_interface_on_runtime(interface,
                                                              klass);
}
