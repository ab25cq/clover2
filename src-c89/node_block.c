...
{
    sNodeBlock * block;
    block = xcalloc(1, sizeof(sNodeBlock));
    block->mSizeNodes = 32;
    block->mNumNodes = 0;
    block->mNodes = xcalloc(1,
                            sizeof(unsigned int) * block->mSizeNodes);
    block->mLVTable = (void *) 0;
    block->mUnClosedBlock = 0;
...
static void append_node_to_node_block(sNodeBlock * node_block,
                                      unsigned int node)
{
    if (node_block->mSizeNodes <= node_block->mNumNodes)
    {
        int new_size;
        new_size = node_block->mSizeNodes * 2;
        node_block->mNodes = xrealloc(node_block->mNodes,
                                      sizeof(unsigned int) * new_size);
        memset(node_block->mNodes + node_block->mSizeNodes,
               0,
               sizeof(unsigned int) * (new_size - node_block->mSizeNodes));
    }
...
int parse_block(sNodeBlock * * node_block,
                ...
                int block_object)
{
    sVarTable * old_vtable;
    char * source_head;
    char * source_end;
    *node_block = sNodeBlock_alloc();
    old_vtable = info->lv_table;
    if (new_table)
    {
    ...
    while (1)
    {
        ...
        else if (*info->p == '\000')
        {
            ...
            sBuf_append(&(*node_block)->mSource,
                        source_head,
                        source_end - source_head);
            sBuf_append_char(&(*node_block)->mSource, '\000');
            return 1;
        }
        node = 0;
...
int compile_block(sNodeBlock * block, sCompileInfo * info)
{
    ...
    for (i = 0; i < block->mNumNodes; i++)
    {
        ...
        append_opecode_to_code(info->code, 14, info->no_output);
        stack_num = info->stack_num;
