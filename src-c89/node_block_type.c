...
int CLParser_initialize(CLVALUE * * stack_ptr,
                        CLVALUE * lvar,
                        sVMInfo * info);
static sNodeBlockType * * gNodeBlockTypes = (void *) 0;
static int gUsedPageNodeBlockTypes = 0;
static int gSizePageNodeBlockTypes = 0;
static int gUsedNodeBlockTypes = 0;
void init_node_block_types()
{
    const int size_page_node_block_types;
    size_page_node_block_types = 4;
    if (gSizePageNodeBlockTypes == 0)
    {
        int i;
        gNodeBlockTypes = xcalloc(1,
                                  sizeof(sNodeBlockType *) * size_page_node_block_types);
        for (i = 0; i < size_page_node_block_types; i++)
        {
            gNodeBlockTypes[i] = xcalloc(1, sizeof(sNodeBlockType) * 64);
        }
...
void free_node_block_types()
{
    if (gSizePageNodeBlockTypes > 0)
    {
        ...
        {
            xfree(gNodeBlockTypes[i]);
        }
        xfree(gNodeBlockTypes);
...
sNodeBlockType * alloc_node_block_type()
{
    ...
    if (gUsedNodeBlockTypes == 64)
    {
        ...
        if (gUsedPageNodeBlockTypes == gSizePageNodeBlockTypes)
        {
            int new_size;
            int i;
            new_size = (gSizePageNodeBlockTypes + 1) * 2;
            gNodeBlockTypes = xrealloc(gNodeBlockTypes,
                                       sizeof(sNodeBlockType *) * new_size);
            memset(gNodeBlockTypes + gSizePageNodeBlockTypes,
                   0,
                   sizeof(sNodeBlockType *) * (new_size - gSizePageNodeBlockTypes));
...
int substitution_posibility_for_node_block_type(sNodeBlockType * left_block,
                                                ...
                                                sNodeType * right_generics_types)
{
    if (substitution_posibility(left_block->mResultType,
                                right_block->mResultType,
                                (void *) 0,
                                (void *) 0,
                                (void *) 0,
                                (void *) 0))
    {
        if (left_block->mNumParams == right_block->mNumParams)
        {
            ...
            for (i = 0; i < left_block->mNumParams; i++)
            {
                ...
                if (!substitution_posibility(left_solved_param,
                                             right_solved_param,
                                             (void *) 0,
                                             (void *) 0,
                                             (void *) 0,
