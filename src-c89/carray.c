...
CLObject create_equalable_carray_object(char * type_name)
{
    sCLClass * klass;
    CLObject obj;
    klass = get_class("EqualableArray");
    ;
    obj = create_object(klass, type_name);
    return obj;
}
                                            ...
                                            CLObject * elements)
{
    ...
    if (num_elements > 0)
    {
        sCLObject * obj_data3;
        char * element_type;
        obj_data3 = (sCLObject *) get_object_pointer(elements[0]);
        element_type = obj_data3->mType;
        snprintf(type_name, 128, "Array<%s>", element_type);
    }
    else
    {
        snprintf(type_name, 128, "Array<Null>");
    }
    obj = create_carray_object(type_name);
    cl_value.mLongValue = 0;
    cl_value.mObjectValue = obj;
    push_value_to_global_stack(cl_value);
    object_class = get_class("Object");
    ;
    array = create_array_object(object_class, num_elements);
    obj_data = (sCLObject *) get_object_pointer(obj);
    obj_data->mFields[0].mObjectValue = array;
    obj_data2 = (sCLObject *) get_object_pointer(array);
                             ...
                             sCLClass * class_items)
{
    ...
    char * method_name_and_params;
    sCLMethod * method;
    CLObject items_array;
    sCLObject * object_data2;
    int i;
    klass = get_class("Array");
    method_name_and_params = "initialize(GenericsParametorClass0[])";
    method = search_for_method_from_virtual_method_table(klass,
                                                         method_name_and_params);
    (*stack_ptr)->mObjectValue = array_object;
    (*stack_ptr)++;
    items_array = create_array_object(class_items, num_elements);
    object_data2 = (sCLObject *) get_object_pointer(items_array);
    for (i = 0; i < num_elements; i++)
    {
                                                      ...
                                                      CLObject * elements)
{
    ...
    obj = create_equalable_carray_object(type_name);
    cl_value.mLongValue = 0;
    cl_value.mObjectValue = obj;
    push_value_to_global_stack(cl_value);
    object_class = get_class("Object");
                                                     ...
                                                     CLObject * elements)
{
    ...
    obj = create_sortable_carray_object(type_name);
    cl_value.mLongValue = 0;
    cl_value.mObjectValue = obj;
    push_value_to_global_stack(cl_value);
    object_class = get_class("Object");
