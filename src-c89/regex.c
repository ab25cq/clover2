...
void regex_free_fun(CLObject obj)
{
    ...
    object_data = (sRegexObject *) get_object_pointer(obj);
    pcre_free(object_data->mRegex);
    xfree(object_data->mRegexString);
}
CLObject create_regex_object(char * regex,
                             int global,
                             int ignore_case,
                             ...
                             int ungreedy)
{
    ...
    object_data = (sRegexObject *) get_object_pointer(obj);
    option = ignore_case ? 0x1 : 0;
    options = 0x800 | (ignore_case ? 0x1 : 0) | (multiline ? 0x2 : 0) | (extended ? 0x8 : 0) | (dotall ? 0x4 : 0) | (dollar_endonly ? 0x20 : 0) | (ungreedy ? 0x200 : 0);
    object_data->mRegexString = xstrdup(regex);
    object_data->mRegex = pcre_compile(regex,
                                       options,
                                       &err,
                                       &erro_ofs,
                                       (void *) 0);
...
int regex_equals(CLObject left, CLObject right)
{
    sRegexObject * left_object;
    sRegexObject * right_object;
    left_object = (sRegexObject *) get_object_pointer(left);
    right_object = (sRegexObject *) get_object_pointer(right);
    return strcmp(left_object->mRegexString,
                  right_object->mRegexString) == 0 && left_object->mGlobal == right_object->mGlobal && left_object->mIgnoreCase == right_object->mIgnoreCase && left_object->mMultiline == right_object->mMultiline && left_object->mExtended == right_object->mExtended && left_object->mDotAll == right_object->mDotAll && left_object->mAnchored == right_object->mAnchored && left_object->mDollarEndOnly == right_object->mDollarEndOnly && left_object->mUngreedy == right_object->mUngreedy;
}
