...
static void node_type_to_cl_type(sNodeType * node_type,
                                 sCLType * * cl_type,
                                 sCLClass * klass)
{
    int i;
    *cl_type = create_cl_type(node_type->mClass, klass);
    ...
    {
        node_type_to_cl_type(node_type->mGenericsTypes[i],
                             &(*cl_type)->mGenericsTypes[i],
                             klass);
    }
    ...
    {
        int i;
        (*cl_type)->mBlockType = xcalloc(1, sizeof(sCLBlockType));
        (*cl_type)->mBlockType->mNumParams = node_type->mBlockType->mNumParams;
        for (i = 0; i < node_type->mBlockType->mNumParams; i++)
        {
            node_type_to_cl_type(node_type->mBlockType->mParams[i],
                                 &(*cl_type)->mBlockType->mParams[i],
                                 klass);
        }
        node_type_to_cl_type(node_type->mBlockType->mResultType,
                             &(*cl_type)->mBlockType->mResultType,
                             klass);
    }
    else
    {
        (*cl_type)->mBlockType = (void *) 0;
    }
}
static void parser_param_to_cl_param(sParserParam * param,
                                     sCLParam * type,
                                     sCLClass * klass)
{
    type->mNameOffset = append_str_to_constant_pool(&klass->mConst,
                                                    param->mName,
                                                    0);
    node_type_to_cl_type(param->mType, &type->mType, klass);
    type->mDefaultValueOffset = append_str_to_constant_pool(&klass->mConst,
                                                            param->mDefaultValue,
                                                            0);
}
static void create_method_path(char * result,
                               int result_size,
                               sCLMethod * method,
                               sCLClass * klass)
{
    ...
    {
        xstrncat(result,
                 (char *) ((&klass->mConst)->mConst + method->mParams[i].mType->mClassNameOffset),
                 result_size);
                                   ...
                                   int num_params)
{
    ...
    *result = 0;
    ...
    {
        sNodeType * param_type;
        sCLClass * klass2;
        int array;
        int nullable;
        param_type = param_types[i];
        klass2 = param_type->mClass;
        array = param_type->mArray;
        nullable = param_type->mNullable;
        if (klass2 == klass)
        {
        ...
        {
            xstrncat(result,
                     (char *) ((&klass2->mConst)->mConst + klass2->mClassNameOffset),
                     size_result);
        }
        if (array)
        {
...
void set_method_index_to_class(sCLClass * klass)
{
    ...
    for (i = klass->mNumMethods - 1; i >= 0; i--)
    {
        ...
        if (method->mFlags & 0x2 && strcmp((char *) ((&klass->mConst)->mConst + method->mNameOffset),
                                           "initialize") == 0 && method->mNumParams == 0)
        {
    ...
    for (i = klass->mNumMethods - 1; i >= 0; i--)
    {
        ...
        method = klass->mMethods + i;
        if (method->mFlags & 0x2 && strcmp((char *) ((&klass->mConst)->mConst + method->mNameOffset),
                                           "finalize") == 0 && method->mNumParams == 0)
        {
    ...
    for (i = klass->mNumMethods - 1; i >= 0; i--)
    {
        ...
        method = klass->mMethods + i;
        if (method->mFlags & 0x2 && strcmp((char *) ((&klass->mConst)->mConst + method->mNameOffset),
                                           "callingMethod") == 0 && method->mNumParams == 4)
        {
            sNodeType * param1;
            sNodeType * param2;
            sNodeType * param3;
            sNodeType * param4;
            param1 = create_node_type_from_cl_type(method->mParams[0].mType,
                                                   klass);
            param2 = create_node_type_from_cl_type(method->mParams[1].mType,
                                                   klass);
            param3 = create_node_type_from_cl_type(method->mParams[2].mType,
                                                   klass);
            param4 = create_node_type_from_cl_type(method->mParams[3].mType,
                                                   klass);
            if (type_identify_with_class_name(param1,
                                              "String") && type_identify_with_class_name(param2,
                                                                                         "Array") && type_identify_with_class_name(param3,
                                                                                                                                   "int") && type_identify_with_class_name(param4,
                                                                                                                                                                           "int"))
            {
                        ...
                        sGenericsParamInfo * ginfo)
{
    ...
    int method_arg_default_value;
    int i;
    char method_path[1024 + 1];
    ...
    {
        int new_size;
        new_size = klass->mSizeMethods * 2;
        klass->mMethods = xrealloc(klass->mMethods,
                                   sizeof(sCLMethod) * new_size);
        memset(klass->mMethods + klass->mSizeMethods,
               0,
               sizeof(sCLMethod) * (new_size - klass->mSizeMethods));
        klass->mSizeMethods = new_size;
    }
    num_methods = klass->mNumMethods;
    klass->mMethods[num_methods].mFlags = (native_ ? 0x1 : 0) | (static_ ? 0x2 : 0);
    klass->mMethods[num_methods].mNameOffset = append_str_to_constant_pool(&klass->mConst,
                                                                           method_name,
                                                                           0);
    method_arg_default_value = 0;
    for (i = 0; i < num_params; i++)
    {
        sParserParam * param;
        param = params + i;
        if (param->mDefaultValue[0] != '\000')
        {
            method_arg_default_value = 1;
        }
        else
        {
        ...
        parser_param_to_cl_param(param,
                                 klass->mMethods[num_methods].mParams + i,
                                 klass);
    }
    klass->mMethods[num_methods].mNumParams = num_params;
    create_method_path(method_path,
                       1024,
                       &klass->mMethods[num_methods],
                       klass);
    klass->mMethods[num_methods].mPathOffset = append_str_to_constant_pool(&klass->mConst,
                                                                           method_path,
                                                                           0);
    node_type_to_cl_type(result_type,
                         &klass->mMethods[num_methods].mResultType,
                         klass);
    ...
    create_method_name_and_params(method_name_and_params,
                                  ...
                                  param_types,
                                  num_params);
    klass->mMethods[num_methods].mMethodNameAndParamsOffset = append_str_to_constant_pool(&klass->mConst,
    ...
    if (ginfo)
    {
        ...
        {
            char * interface_name;
            interface_name = (char *) ((&ginfo->mInterface[i]->mConst)->mConst + ginfo->mInterface[i]->mClassNameOffset);
            klass->mMethods[num_methods].mGenericsParamTypeOffsets[i] = append_str_to_constant_pool(&klass->mConst,
                                                                                                    interface_name,
                                                                                                    0);
        }
    }
...
{
    sCLClass * klass2;
    klass->mTypedefClassName1Offsets[klass->mNumTypedef] = append_str_to_constant_pool(&klass->mConst,
                                                                                       class_name1,
                                                                                       0);
    klass->mTypedefClassName2Offsets[klass->mNumTypedef] = append_str_to_constant_pool(&klass->mConst,
                                                                                       class_name2,
                                                                                       0);
    klass->mNumTypedef++;
    if (klass->mNumTypedef >= 64)
    {
        return 0;
    }
    klass2 = get_class_with_load_on_compile_time(class_name2);
    if (klass2)
    {
                       ...
                       sNodeType * result_type)
{
    ...
    if (klass->mNumFields == klass->mSizeFields)
    {
        ...
        klass->mFields = xrealloc(klass->mFields,
                                  sizeof(sCLField) * new_size);
        memset(klass->mFields + klass->mSizeFields,
               0,
               sizeof(sCLField) * (new_size - klass->mSizeFields));
        klass->mSizeFields = new_size;
    }
    num_fields = klass->mNumFields;
    klass->mFields[num_fields].mFlags = (private_ ? 0x1 : 0) | (protected_ ? 0x2 : 0);
    klass->mFields[num_fields].mNameOffset = append_str_to_constant_pool(&klass->mConst,
                                                                         name,
                                                                         0);
    klass->mFields[num_fields].mNumDelegatedMethod = 0;
    memset(&klass->mFields[num_fields].mDelegatedMethodIndex,
           0,
           sizeof(int) * 256);
    node_type_to_cl_type(result_type,
                         &klass->mFields[num_fields].mResultType,
                         klass);
    klass->mNumFields++;
    return 1;
}
                             ...
                             int initialize_value)
{
    ...
    {
        int new_size;
        new_size = klass->mSizeClassFields * 2;
        klass->mClassFields = xrealloc(klass->mClassFields,
                                       sizeof(sCLField) * new_size);
        memset(klass->mClassFields + klass->mSizeClassFields,
               0,
               sizeof(sCLField) * (new_size - klass->mSizeClassFields));
        klass->mSizeClassFields = new_size;
    }
    num_fields = klass->mNumClassFields;
    klass->mClassFields[num_fields].mFlags = (private_ ? 0x1 : 0) | (protected_ ? 0x2 : 0);
    klass->mClassFields[num_fields].mNameOffset = append_str_to_constant_pool(&klass->mConst,
                                                                              name,
                                                                              0);
    klass->mClassFields[num_fields].mInitializeValue = initialize_value;
    node_type_to_cl_type(result_type,
                         &klass->mClassFields[num_fields].mResultType,
                         klass);
...
int determine_method_generics_types(sNodeType * left_param,
                                    ...
                                    sNodeType * method_generics_types)
{
    sCLClass * left_param_class;
    sCLClass * right_param_class;
    int left_param_class_num;
    int right_param_class_num;
    int i;
    sNodeBlockType * left_block_type;
    sNodeBlockType * right_block_type;
    left_param_class = left_param->mClass;
    right_param_class = right_param->mClass;
    left_param_class_num = left_param_class->mMethodGenericsParamClassNum;
    right_param_class_num = right_param_class->mMethodGenericsParamClassNum;
    ...
    {
        if (!determine_method_generics_types(left_param->mGenericsTypes[i],
                                             right_param->mGenericsTypes[i],
                                             method_generics_types))
        {
            return 0;
        }
    }
    left_block_type = left_param->mBlockType;
    right_block_type = right_param->mBlockType;
    if (left_block_type != (void *) 0)
    {
        for (i = 0; i < left_block_type->mNumParams; i++)
        {
            if (!determine_method_generics_types(left_block_type->mParams[i],
                                                 right_block_type->mParams[i],
                                                 method_generics_types))
            {
                return 0;
            }
        }
        if (!determine_method_generics_types(left_block_type->mResultType,
                                             right_block_type->mResultType,
                                             method_generics_types))
        {
            return 0;
        }
...
static int search_for_class_file_on_compile_time(char * class_name,
                                                 char * class_file_path,
                                                 size_t class_file_path_size)
{
    char * home;
    char * cwd;
    home = getenv("HOME");
    if (home)
    {
        snprintf(class_file_path,
                 class_file_path_size,
                 "%s/.clover2/%s.oclcl",
                 home,
                 class_name);
        if (access(class_file_path, 0) == 0)
        {
            return 1;
        }
    }
    ...
    if (cwd)
    {
        ...
        else
        {
            ...
            char cclover2_path[4096];
            char * compiling_source_dir;
            snprintf(source_path, 4096, "%s/%s.clcl", cwd, class_name);
            snprintf(cclover2_path, 4096, "%s/cclover2", cwd);
            if (access(source_path, 0) == 0)
            {
                if (access(cclover2_path, 1) == 0)
                {
                    char command[4096 * 2];
                    int rc;
                    snprintf(command,
            ...
            compiling_source_dir = dirname(gCompilingSourceFileName);
            snprintf(source_path,
                     4096,
                     "%s/%s.clcl",
                     compiling_source_dir,
                     class_name);
            if (access(source_path, 0) == 0)
            {
                if (access(cclover2_path, 1) == 0)
...
{
    sCLClass * klass;
    char class_file_name[4096 + 1];
    klass = get_class(class_name);
    if (klass != (void *) 0)
    {
        remove_class(class_name);
    }
    if (!search_for_class_file_on_compile_time(class_name,
                                               class_file_name,
                                               4096))
    {
        return (void *) 0;
...
static int check_method_params(sCLMethod * method,
                               sCLClass * klass,
                               char * method_name,
                               sNodeType * * param_types,
                               int num_params,
                               ...
                               int lazy_lambda_compile)
{
    if (strcmp((char *) ((&klass->mConst)->mConst + method->mNameOffset),
               method_name) == 0)
    {
        if (search_for_class_method && method->mFlags & 0x2 || !search_for_class_method && !(method->mFlags & 0x2))
        {
            if (num_params == method->mNumParams)
            {
                ...
                for (j = 0; j < num_params; j++)
                {
                    ...
                    {
                        if (!substitution_posibility(param,
                                                     param_types[j],
                                                     left_generics_type,
                                                     right_generics_type,
                                                     left_method_generics,
                                                     right_method_generics))
                        {
                            return 0;
                        }
                        if (!determine_method_generics_types(param,
                                                             param_types[j],
                                                             method_generics_types))
                        {
                            return 0;
...
static sNodeType * get_method_genercs_from_method(sCLClass * klass,
                                                  sCLMethod * method)
{
    sNodeType * result;
    int i;
    result = alloc_node_type();
    result->mClass = (void *) 0;
    for (i = 0; i < method->mNumGenerics; i++)
    {
        char * interface_name;
        interface_name = (char *) ((&klass->mConst)->mConst + method->mGenericsParamTypeOffsets[i]);
...
int search_for_method(sCLClass * klass,
                      ...
                      sNodeType * * result_type,
                      int lazy_lambda_compile,
                      int lazy_lambda_compile2,
                      sNodeType * * method_generics_types)
{
    int i;
    if (*method_generics_types == (void *) 0)
    {
    ...
    if (start_point < klass->mNumMethods)
    {
        for (i = start_point; i >= 0; i--)
        {
            ...
            sNodeType * left_method_generics;
            method = klass->mMethods + i;
            left_method_generics = get_method_genercs_from_method(klass,
                                                                  method);
            if (lazy_lambda_compile2)
            {
                right_method_generics = *method_generics_types;
            }
            if (check_method_params(method,
                                    klass,
                                    method_name,
                                    param_types,
                                    num_params,
                                    search_for_class_method,
                                    left_generics_type,
                                    right_generics_type,
                                    left_method_generics,
                                    right_method_generics,
                                    ...
                                    lazy_lambda_compile))
            {
                ...
                result_type2 = create_node_type_from_cl_type(method->mResultType,
                                                             klass);
                ...
                if (!solve_generics_types_for_node_type(result_type3,
                                                        result_type,
                                                        left_generics_type,
                                                        1,
                                                        0))
    ...
    *num_methods = 0;
    if (start_point < klass->mNumMethods)
    {
        for (i = start_point; i >= 0; i--)
        {
            ...
            method = klass->mMethods + i;
            if (strcmp((char *) ((&klass->mConst)->mConst + method->mNameOffset),
                       method_name) == 0 && (method->mFlags & 0x2 ? 1 : 0) == class_method)
            {
...
int search_for_field(sCLClass * klass, char * field_name)
{
    ...
    for (i = 0; i < klass->mNumFields; i++)
    {
        ...
        if (strcmp((char *) ((&klass->mConst)->mConst + field->mNameOffset),
                   field_name) == 0)
        {
            return i;
        }
...
static int check_same_interface_of_two_methods(sCLMethod * method1,
                                               sCLClass * klass1,
                                               sCLMethod * method2,
                                               sCLClass * klass2)
{
    char * name1;
    char * name2;
    sNodeType * result_type1;
    sNodeType * result_type2;
    int i;
    name1 = (char *) ((&klass1->mConst)->mConst + method1->mNameOffset);
    name2 = (char *) ((&klass2->mConst)->mConst + method2->mNameOffset);
    if (strcmp(name1, name2) != 0)
    {
        return 0;
    }
    result_type1 = create_node_type_from_cl_type(method1->mResultType,
                                                 klass1);
    result_type2 = create_node_type_from_cl_type(method2->mResultType,
                                                 klass2);
    if (!type_identify_with_class_name(result_type1,
                                       "Self") && !type_identify_with_class_name(result_type2,
                                                                                 "Self"))
    {
    ...
    for (i = 0; i < method1->mNumParams; i++)
    {
        sCLParam * param1;
        sCLParam * param2;
        sNodeType * param1_type;
        sNodeType * param2_type;
        param1 = method1->mParams + i;
        param2 = method2->mParams + i;
        param1_type = create_node_type_from_cl_type(param1->mType, klass1);
        param2_type = create_node_type_from_cl_type(param2->mType, klass2);
        if (!type_identify_with_class_name(param1_type,
                                           "Self") && !type_identify_with_class_name(param2_type,
                                                                                     "Self"))
        {
...
int check_implemented_methods_for_interface(sCLClass * left_class,
                                            sCLClass * right_class)
{
    sCLClass * anonymous_class;
    anonymous_class = get_class("Anonymous");
    if (right_class == anonymous_class)
    {
        return 1;
    }
    else if (right_class->mFlags & 0x1)
    {
        return 0;
    }
    else if (left_class != right_class)
    {
        ...
        for (i = 0; i < left_class->mNumMethods; i++)
        {
            ...
            int found;
            int j;
            method = left_class->mMethods + i;
            found = 0;
            for (j = 0; j < right_class->mNumMethods; j++)
            {
                sCLMethod * method2;
                method2 = right_class->mMethods + j;
                if (check_same_interface_of_two_methods(method,
                                                        left_class,
                                                        method2,
                                                        right_class))
                {
...
static void append_const_to_buffer(sBuf * buf,
                                   sConst * constant,
                                   sCLClass * klass)
{
...
static void append_cl_type_to_buffer(sBuf * buf,
                                     sCLType * cl_type);
static void append_cl_block_type(sBuf * buf,
                                 sCLBlockType * cl_block_type)
{
    int i;
...
static void append_byte_codes_to_buffer(sBuf * buf,
                                        sByteCode * code)
{
    sBuf_append_int(buf, code->mLen);
    sBuf_append(buf, code->mCodes, code->mLen);
}
static void append_methods_to_buffer(sBuf * buf,
                                     sCLMethod * methods,
                                     sCLClass * klass,
                                     int num_methods)
{
    ...
    for (i = 0; i < num_methods; i++)
    {
        ...
        {
            int n;
            n = method->mGenericsParamTypeOffsets[j];
            sBuf_append_int(buf, n);
        }
    }
}
static void append_fields_to_buffer(sBuf * buf,
                                    sCLField * fields,
                                    int num_fields)
{
    int i;
...
int write_class_to_class_file(sCLClass * klass)
{
    sBuf buf;
    ...
    snprintf(file_name,
             ...
             "%s.oclcl",
             (char *) ((&klass->mConst)->mConst + klass->mClassNameOffset));
    f = open(file_name, 01 | 01000 | 0100, 0644);
    total_size = 0;
    while (total_size < buf.mLen)
    {
        int size;
        if (buf.mLen - total_size < 8192)
        {
            size = write(f, buf.mBuf + total_size, buf.mLen - total_size);
        }
        else
        {
            size = write(f, buf.mBuf + total_size, 8192);
        }
        if (size < 0)
        {
            xfree(buf.mBuf);
...
int write_all_modified_classes()
{
    ...
    while (p)
    {
        ...
        if (klass->mFlags & 0x4)
        {
            ...
            {
                fprintf(stderr,
                        "Clover failed to write class file(%s)\n",
                        (char *) ((&klass->mConst)->mConst + klass->mClassNameOffset));
                return 0;
            }
        }
