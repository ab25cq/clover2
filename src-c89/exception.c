                                            ...
                                            char * msg, ...)
{
    ...
    if (info->running_class && info->running_method)
    {
        ...
        snprintf(msg3,
                 ...
                 info->sline,
                 msg2,
                 (char *) ((&info->running_class->mConst)->mConst + info->running_class->mClassNameOffset),
                 (char *) ((&info->running_class->mConst)->mConst + info->running_method->mNameOffset));
        for (i = 0; i < info->num_stack_trace; i++)
        {
            ...
            snprintf(buf,
                     1024,
                     "(%s %d)",
                     info->stack_trace_sname[i],
                     info->stack_trace_sline[i]);
    ...
    xstrncpy(info->exception_message, msg3, 1024);
    klass = get_class(class_name);
    if (klass == (void *) 0)
    {
        fprintf(stderr,
                "unexpected error. abort on entry_exception_object_with_class_name. The class name is %s.\n",
                class_name);
        exit(2);
    }
    object = create_object(klass, class_name);
    *stack_ptr = stack + var_num;
    (*stack_ptr)->mObjectValue = object;
    (*stack_ptr)++;
    str = create_string_object(info->exception_message);
...
void entry_exception_object(CLObject exception, sVMInfo * info)
{
    ...
    CLObject message;
    char * str;
    object_data = (sCLObject *) get_object_pointer(exception);
    message = object_data->mFields[0].mObjectValue;
    str = string_object_to_char_array(message);
    if (info->running_class && info->running_method)
    {
        int i;
        snprintf(info->exception_message,
                 1024,
                 "%s %d: %s at %s.%s",
                 info->sname,
                 info->sline,
        ...
        for (i = 0; i < info->num_stack_trace; i++)
        {
            ...
            snprintf(buf,
                     ...
                     info->stack_trace_sname[i],
                     info->stack_trace_sline[i]);
            xstrncat(info->exception_message, buf, 1024);
        }
