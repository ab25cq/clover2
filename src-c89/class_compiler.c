...
int CLParser_initialize(CLVALUE * * stack_ptr,
                        CLVALUE * lvar,
                        sVMInfo * info);
static int skip_block(sParserInfo * info)
{
    if (*info->p == '{')
    {
        int dquort;
        ...
        while (1)
        {
            if (dquort)
            {
                if (*info->p == '\\')
                {
                    ...
                    {
                        fprintf(stderr,
                                "%s %d: unexpected the source end. close single quote or double quote.",
                                info->sname,
                                sline);
                        return 0;
                    }
            ...
            {
                parser_err_msg(info,
                               "The block requires } character for closing block");
                info->err_num++;
                return 1;
            }
            else if (*info->p == '\n')
...
static int parse_generics_params(sGenericsParamInfo * ginfo,
                                 sParserInfo * info,
                                 sCompileInfo * cinfo)
{
    ginfo->mNumParams = 0;
    if (*info->p == '<')
    {
        ...
        while (1)
        {
            ...
            {
                char buf[64];
                int num_generics_params;
                sCLClass * interface;
                if (!parse_word(buf, 64, info, 1, 0))
                {
                    return 0;
                }
                num_generics_params = ginfo->mNumParams;
                xstrncpy(ginfo->mParamNames[num_generics_params], buf, 64);
                ...
                if (interface && !(interface->mFlags & 0x2))
                {
                    parser_err_msg(info,
                                   "This is not interface(%s)\n",
                                   (char *) ((&ginfo->mInterface[num_generics_params]->mConst)->mConst + ginfo->mInterface[num_generics_params]->mClassNameOffset));
                    info->err_num++;
                }
                ginfo->mInterface[num_generics_params] = interface;
                ginfo->mNumParams++;
                if (ginfo->mNumParams >= 32)
                {
...
static int parse_class_name_and_attributes(char * class_name,
                                           int class_name_size,
                                           sParserInfo * info,
                                           sCompileInfo * cinfo)
{
    ...
    if (*info->p == ':')
    {
        ...
        while (1)
        {
            ...
            {
                parser_err_msg(info,
                               "It is the source end. Close class definition");
                info->err_num++;
                return 1;
            }
            else if (*info->p == '{')
...
static int parse_class_on_alloc_classes_phase(sParserInfo * info,
                                              sCompileInfo * cinfo,
                                              int interface,
                                              int dynamic_class)
{
    char class_name[64];
    if (!parse_class_name_and_attributes(class_name, 64, info, cinfo))
    {
        return 0;
    }
    info->klass = get_class(class_name);
    if (info->klass == (void *) 0)
    {
        info->klass = alloc_class(class_name,
                                  0,
                                  -1,
                                  -1,
                                  info->generics_info.mNumParams,
                                  info->generics_info.mInterface,
                                  interface,
                                  dynamic_class,
                                  0);
        info->klass->mFlags |= 0x8;
    }
    info->klass->mFlags |= 0x4;
    if (!skip_block(info))
    {
        return 0;
    }
...
static int parse_throws(sParserInfo * info,
                        sCompileInfo * cinfo,
                        int * throw_existance)
{
    char * p_saved;
...
static int parse_method_name_and_params(char * method_name,
                                        int method_name_max,
                                        sParserParam * params,
                                        int * num_params,
                                        sNodeType * * result_type,
                                        ...
                                        sCompileInfo * cinfo)
{
    ...
    if (*info->p == '<')
    {
        ...
        info->method_generics_info = ginfo;
    }
    else
    {
        memset(&info->method_generics_info, 0, sizeof(sGenericsParamInfo));
    }
    if (!parse_word(method_name, method_name_max, info, 1, 0))
    {
        return 0;
    }
    expect_next_character_with_one_forward("(", info);
...
static int parse_field_attributes_and_type(int * private_,
                                           int * protected_,
                                           int * static_,
                                           int * delegate_,
                                           sNodeType * * result_type,
...
static int field_delegation(sParserInfo * info,
                            sCompileInfo * cinfo,
                            sCLClass * klass,
                            sCLField * field)
{
    ...
    field_type = create_node_type_from_cl_type(field->mResultType,
                                               klass);
    field_class = field_type->mClass;
    if (info->err_num == 0 && info->klass->mFlags & 0x8)
    {
        int i;
        ...
        {
            sCLMethod * method;
            method = field_class->mMethods + i;
            if (!(method->mFlags & 0x2) && !(method->mFlags & 0x1) && strcmp((char *) ((&field_class->mConst)->mConst + method->mNameOffset),
                                                                             "initialize") != 0 && strcmp((char *) ((&field_class->mConst)->mConst + method->mNameOffset),
                                                                                                          "finalize") != 0)
            {
                char * method_name;
                sNodeType * params[32];
                int j;
                sParserParam parser_params[32];
                int num_params;
                sNodeType * result_type;
                int native_;
                int static_;
                sGenericsParamInfo method_generics_info;
                int num_methods;
                method_name = (char *) ((&field_class->mConst)->mConst + method->mNameOffset);
                for (j = 0; j < method->mNumParams; j++)
                {
                    sCLParam * param;
                    param = method->mParams + j;
                    params[j] = create_node_type_from_cl_type(param->mType,
                                                              field_class);
                }
                for (j = 0; j < method->mNumParams; j++)
                {
                    char param_name[64];
                    snprintf(param_name, 64, "param%d", j);
                    xstrncpy(parser_params[j].mName, param_name, 64);
                ...
                for (j = 0; j < method_generics_info.mNumParams; j++)
                {
                    ...
                    char * interface_name;
                    sCLClass * interface;
                    offset = method->mGenericsParamTypeOffsets[j];
                    interface_name = (char *) ((&field_class->mConst)->mConst + offset);
                    interface = get_class(interface_name);
                    ;
                    method_generics_info.mInterface[j] = interface;
                    xstrncpy(method_generics_info.mParamNames[j], "dummy", 64);
                }
                if (!add_method_to_class(klass,
                                         method_name,
                                         parser_params,
                                         num_params,
                                         result_type,
                                         native_,
                                         static_,
                                         &method_generics_info))
                {
                    return 0;
                }
                num_methods = field->mNumDelegatedMethod;
                field->mDelegatedMethodIndex[num_methods] = klass->mNumMethods - 1;
...
static int setter_and_getter(sParserInfo * info,
                             sCompileInfo * cinfo,
                             sCLClass * klass,
                             sCLField * field,
                             char * field_name,
                             int private_)
{
    ...
    if (info->err_num == 0 && info->klass->mFlags & 0x8)
    {
        ...
        if (!private_)
        {
            ...
            method_name = field_name;
            num_params = 1;
            xstrncpy(parser_params[0].mName, "value", 64);
            parser_params[0].mType = field_type;
            parser_params[0].mDefaultValue[0] = '\000';
            result_type = create_node_type_with_class_name("Null");
            native_ = 0;
            static_ = 0;
            if (!add_method_to_class(klass,
...
static int parse_methods_and_fields(sParserInfo * info,
                                    sCompileInfo * cinfo,
                                    int interface)
{
    int native_;
    ...
    if (strcmp(buf, "include") == 0)
    {
        ...
        sCLModule * module;
        if (!parse_word(module_name, 64, info, 1, 0))
        {
            return 0;
        }
        ...
        {
            parser_err_msg(info,
                           "The module named %s is not defined",
                           module_name);
            info->err_num++;
        }
        else
        {
            ...
            char * sname_saved;
            int err_num_saved;
            int cinfo_err_num_saved;
            body = get_module_body(module);
            p_saved = info->p;
            sline_saved = info->sline;
            sname_saved = info->sname;
            err_num_saved = info->err_num;
            cinfo_err_num_saved = info->cinfo->err_num;
            info->p = body;
    ...
    else if (strcmp(buf, "def") == 0)
    {
        ...
        native_ = 0;
        static_ = 0;
        if (!parse_method_name_and_params(method_name,
                                          64,
                                          params,
                                          &num_params,
                                          &result_type,
        ...
        if (info->err_num == 0 && info->klass->mFlags & 0x8)
        {
            if (!add_method_to_class(info->klass,
                                     method_name,
                                     params,
                                     num_params,
                                     result_type,
    ...
    {
        char class_name1[64 + 1];
        char class_name2[64 + 1];
        if (!parse_word(class_name1, 64, info, 1, 0))
        {
            return 0;
        }
        skip_spaces_and_lf(info);
        if (!parse_word(class_name2, 64, info, 1, 0))
        {
            return 0;
        }
        skip_spaces_and_lf(info);
        ...
        if (info->err_num == 0)
        {
            ...
            {
                parser_err_msg(info,
                               "overflow typedef number or invalid class name");
                info->err_num++;
                return 0;
            }
        }
    ...
    else if (strcmp(buf, "enum") == 0)
    {
        ...
        while (1)
        {
            char element_name[64 + 1];
            int private_;
            int protected_;
            sNodeType * field_type;
            if (!parse_word(element_name, 64, info, 1, 0))
            {
                return 0;
            }
            private_ = 0;
            protected_ = 0;
            field_type = create_node_type_with_class_name("int");
            if (!add_class_field_to_class(info->klass,
                                          element_name,
                                          private_,
                                          protected_,
                                          field_type,
    ...
    else
    {
        ...
        result_type = (void *) 0;
        expect_next_character_with_one_forward(":", info);
        if (!parse_field_attributes_and_type(&private_,
                                             &protected_,
                                             &static_,
                                             &delegate_,
                                             &result_type,
        ...
        if (info->err_num == 0 && info->klass->mFlags & 0x8)
        {
            ...
            {
                if (!add_field_to_class(info->klass,
                                        buf,
                                        private_,
                                        protected_,
                                        result_type))
...
static int parse_class_on_add_methods_and_fields(sParserInfo * info,
                                                 sCompileInfo * cinfo,
                                                 int interface)
{
    char class_name[64];
    ...
    else
    {
        while (1)
        {
            ...
            {
                parser_err_msg(info,
                               "It is the source end. Close class definition with }");
                info->err_num++;
                return 1;
            }
            else if (*info->p == '}')
...
static int setter_and_getter_on_compile_time(sParserInfo * info,
                                             sCompileInfo * cinfo,
                                             sCLClass * klass,
                                             sCLField * field,
                                             char * field_name,
                                             int private_)
{
    ...
    if (info->err_num == 0 && info->klass->mFlags & 0x8)
    {
        ...
        info2.get_type_for_interpreter = 0;
        info2.next_command_is_to_bool = 0;
        info2.exist_block_object_err = 0;
        method = klass->mMethods + field->mNumGetterMethodIndex;
        ...
        if (!private_)
        {
            ...
            info2.next_command_is_to_bool = 0;
            info2.exist_block_object_err = 0;
            method = klass->mMethods + field->mNumGetterMethodIndex + 1;
            if (!compile_method(method,
                                parser_params,
                                num_params,
                                &info2,
...
static int field_delegation_on_compile_time(sParserInfo * info,
                                            sCompileInfo * cinfo,
                                            sCLClass * klass,
                                            sCLField * field,
                                            char * field_name)
{
    ...
    if (info->err_num == 0 && info->klass->mFlags & 0x8)
    {
        ...
        for (i = 0; i < field->mNumDelegatedMethod; i++)
        {
            ...
            sParserInfo info2;
            char source[1024];
            method = klass->mMethods + field->mDelegatedMethodIndex[i];
            ...
            for (j = 0; j < method_generics_info.mNumParams; j++)
            {
                ...
                sCLClass * interface;
                offset = method->mGenericsParamTypeOffsets[j];
                interface_name = (char *) ((&klass->mConst)->mConst + offset);
                interface = get_class(interface_name);
                ;
                method_generics_info.mInterface[j] = interface;
                xstrncpy(method_generics_info.mParamNames[j], "dummy", 64);
...
int parse_methods_and_fields_on_compile_time(sParserInfo * info,
                                             sCompileInfo * cinfo,
                                             int interface)
{
    char buf[64];
    ...
    if (strcmp(buf, "include") == 0)
    {
        ...
        else
        {
            ...
            {
                skip_spaces_and_lf(info);
                if (!parse_methods_and_fields_on_compile_time(info,
                                                              cinfo,
                                                              interface))
                {
                    return 0;
    ...
    else if (strcmp(buf, "def") == 0)
    {
        ...
        method = info->klass->mMethods + info->klass->mMethodIndexOnCompileTime;
        info->klass->mMethodIndexOnCompileTime++;
        if (info->klass->mFlags & 0x8)
        {
            if (native_ || interface)
            {
                if (*info->p == ';')
    ...
    else
    {
        ...
        if (info->klass->mFlags & 0x8)
        {
            ...
            if (field_index != -1)
            {
                sCLField * field;
                field = klass->mFields + field_index;
                if (!setter_and_getter_on_compile_time(info,
                                                       cinfo,
                                                       klass,
                                                       field,
                                                       field_name,
            ...
            if (delegate_)
            {
                if (field_index != -1)
                {
                    sCLField * field;
                    field = klass->mFields + field_index;
                    if (!field_delegation_on_compile_time(info,
                                                          cinfo,
                                                          klass,
                                                          field,
                                                          field_name))
...
static int parse_class_on_compile_code(sParserInfo * info,
                                       sCompileInfo * cinfo,
                                       int interface)
{
    char class_name[64];
...
static int parse_class(sParserInfo * info,
                       sCompileInfo * cinfo,
                       int interface,
                       int dynamic_class)
{
    ...
    {
    case 1:
        if (!parse_class_on_alloc_classes_phase(info,
                                                cinfo,
                                                interface,
                                                dynamic_class))
        {
...
static int parse_module(sParserInfo * info, sCompileInfo * cinfo)
{
    char module_name[64];
    if (!parse_word(module_name, 64, info, 1, 0))
    {
    ...
    if (info->parse_phase == 1)
    {
        ...
        {
            parser_err_msg(info,
                           "expected that next character is {, but it is %c(%d)",
                           *info->p,
                           *info->p);
            info->err_num++;
            info->p++;
        ...
        module = create_module(module_name, sname, sline);
        if (module == (void *) 0)
        {
            parser_err_msg(info,
                           "overflow the module table or the same name module exists(%s)",
                           module_name);
            return 0;
        }
...
static int parse_class_source(sParserInfo * info,
                              sCompileInfo * cinfo);
static int include_file(sParserInfo * info, sCompileInfo * cinfo)
{
    char file_name[4096 + 1];
    char * p;
    expect_next_character_with_one_forward("\"", info);
    p = file_name;
    if (*info->p != '/')
    ...
    while (1)
    {
        ...
        {
            parser_err_msg(info,
                           "requires to close \" for including file name");
            return 0;
        }
        else
        {
    ...
    *p = '\000';
    if (strcmp(gCompilingSourceFileName, file_name) != 0)
    {
        sBuf source;
        sBuf source2;
        char * info_p_before;
        char * info_sname_before;
        int info_sline_before;
        int info_included_source_before;
        sBuf_init(&source);
        ...
        info_p_before = info->p;
        info->p = source2.mBuf;
        info_sname_before = info->sname;
        info->sname = file_name;
        info_sline_before = info->sline;
        info->sline = 1;
        info_included_source_before = info->included_source;
        info->included_source = 1;
...
int compile_class_source(char * fname, char * source)
{
    ...
    info.sname = fname;
    info.sline = 1;
    info.lv_table = (void *) 0;
    info.parse_phase = 0;
    info.included_source = 0;
    append_cwd_for_path(fname, fname2);
    xstrncpy(gCompilingSourceFileName, fname2, 4096);
    memset(&cinfo, 0, sizeof(sCompileInfo));
    cinfo.code = (void *) 0;
    cinfo.constant = (void *) 0;
    cinfo.lv_table = (void *) 0;
    cinfo.no_output = 0;
    cinfo.pinfo = &info;
    info.cinfo = &cinfo;
    for (i = 1; i < 8; i++)
