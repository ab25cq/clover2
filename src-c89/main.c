...
int main(int argc, char * * argv, char * const * envp)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        base_name[p - source] = '\000';
        snprintf(object_file_name, 4096, "%s.ocl", base_name);
        if (strcmp(source, object_file_name) != 0)
        {
            if (access(object_file_name, 4) != 0)
            {
                char cmd[4096 + 20];
                char command_path[4096 + 1];
                int rc;
                snprintf(command_path, 4096, "./cclover2");
            ...
            else
            {
                ...
                if (stat(source, &source_stat) == 0 && stat(object_file_name,
                                                            &object_file_stat) == 0)
                {
                    if (source_stat.st_mtim.tv_sec >= object_file_stat.st_mtim.tv_sec)
                    {
                        char cmd[4096 + 20];
                        char command_path[4096 + 1];
                        int rc;
        ...
        clover2_init();
        if (!eval_file(object_file_name, 1024))
        {
