...
int CLParser_initialize(CLVALUE * * stack_ptr,
                        CLVALUE * lvar,
                        sVMInfo * info);
static sNodeType * * gNodeTypes = (void *) 0;
static int gUsedPageNodeTypes = 0;
static int gSizePageNodeTypes = 0;
static int gUsedNodeTypes = 0;
void init_node_types()
{
    const int size_page_node_types;
    size_page_node_types = 4;
    if (gSizePageNodeTypes == 0)
    {
        int i;
        gNodeTypes = xcalloc(1,
                             sizeof(sNodeType *) * size_page_node_types);
        for (i = 0; i < size_page_node_types; i++)
        {
            gNodeTypes[i] = xcalloc(1, sizeof(sNodeType) * 64);
        }
...
sNodeType * alloc_node_type()
{
    ...
    if (gUsedNodeTypes == 64)
    {
        ...
        if (gUsedPageNodeTypes == gSizePageNodeTypes)
        {
            ...
            memset(gNodeTypes + gSizePageNodeTypes,
                   0,
                   sizeof(sNodeType *) * (new_size - gSizePageNodeTypes));
...
sNodeType * clone_node_type(sNodeType * node_type)
{
    ...
    {
        node_type2->mGenericsTypes[i] = clone_node_type(node_type->mGenericsTypes[i]);
    }
...
sNodeType * create_node_type_with_class_pointer(sCLClass * klass)
{
    ...
    node_type->mArray = 0;
    node_type->mNullable = 0;
    node_type->mBlockType = (void *) 0;
    return node_type;
}
...
static sNodeType * parse_class_name(char * * p,
                                    char * * p2,
                                    char * buf,
                                    sParserInfo * info)
{
    ...
    while (* (*p))
    {
        if (* (*p) == '<')
        {
            ...
            * (*p2) = 0;
            ...
            while (1)
            {
                node_type->mGenericsTypes[node_type->mNumGenericsTypes] = parse_class_name(p,
                                                                                           p2,
                                                                                           buf,
                                                                                           info);
                node_type->mNumGenericsTypes++;
                if (node_type->mNumGenericsTypes >= 32)
                {
                    return (void *) 0;
                }
                if (* (*p) == ',')
...
sNodeType * create_node_type_with_class_name(char * class_name)
{
    ...
    p = class_name;
    p2 = buf;
    return parse_class_name(&p, &p2, buf, (void *) 0);
}
sNodeType * create_node_type_with_generics_number(int generics_num)
{
    char class_name[64 + 1];
    snprintf(class_name, 64, "GenericsParametorClass%d", generics_num);
...
sNodeType * create_node_type_with_method_generics_number(int generics_num)
{
    ...
    snprintf(class_name,
             64,
             "MethodGenericsParametorClass%d",
             generics_num);
    return create_node_type_with_class_name(class_name);
}
sNodeType * create_node_type_from_cl_type(sCLType * cl_type,
                                          sCLClass * klass)
{
    ...
    int i;
    node_type = alloc_node_type();
    node_type->mClass = get_class_with_load_on_compile_time((char *) ((&klass->mConst)->mConst + cl_type->mClassNameOffset));
    ;
    node_type->mNumGenericsTypes = cl_type->mNumGenericsTypes;
    for (i = 0; i < cl_type->mNumGenericsTypes; i++)
    {
        node_type->mGenericsTypes[i] = create_node_type_from_cl_type(cl_type->mGenericsTypes[i],
                                                                     klass);
    }
    ...
    if (cl_type->mBlockType)
    {
        ...
        {
            node_type->mBlockType->mParams[i] = create_node_type_from_cl_type(cl_type->mBlockType->mParams[i],
                                                                              klass);
        }
        node_type->mBlockType->mResultType = create_node_type_from_cl_type(cl_type->mBlockType->mResultType,
                                                                           klass);
    }
    else
    {
                            ...
                            sNodeType * right_method_generics)
{
    ...
    {
        if (!solve_generics_types_for_node_type(left,
                                                &left2,
    ...
    {
        if (!solve_generics_types_for_node_type(right,
                                                &right2,
    ...
    {
        if (!solve_generics_types_for_node_type(left2,
                                                &left3,
                                                left_generics_types,
                                                1,
                                                0))
    ...
    {
        if (!solve_generics_types_for_node_type(right2,
                                                &right3,
                                                right_generics_types,
                                                1,
                                                0))
        {
            return 0;
    ...
    left_class = left3->mClass;
    right_class = right3->mClass;
    if (left_class->mGenericsParamClassNum != -1 || right_class->mGenericsParamClassNum != -1)
    {
        return 0;
    }
    else if (type_identify_with_class_name(right3,
                                           "Null") && (!(left_class->mFlags & 0x1) || left3->mArray) && left->mNullable)
    {
        return 1;
    }
    else if (type_identify_with_class_name(right3,
                                           "Anonymous") && !(left_class->mFlags & 0x1))
    {
        return 1;
    }
    else if (type_identify_with_class_name(right3,
                                           "WildCard") && !(left_class->mFlags & 0x1))
    {
        return 1;
    }
    else if (type_identify_with_class_name(left3,
                                           "Anonymous") && !(right_class->mFlags & 0x1))
    {
        return 1;
    }
    else if (left_class->mFlags & 0x2)
    {
        if (right_class->mFlags & 0x2)
        {
        ...
        {
            return check_implemented_methods_for_interface(left_class,
                                                           right_class);
        }
    }
    else if (type_identify_with_class_name(left3, "lambda"))
    {
        if (type_identify_with_class_name(right3, "lambda"))
        {
            ...
            left_block_type = left3->mBlockType;
            right_block_type = right3->mBlockType;
            if (left_block_type && right_block_type)
            {
                return substitution_posibility_for_node_block_type(left_block_type,
                                                                   right_block_type,
                                                                   left_generics_types,
                                                                   right_generics_types);
            }
    ...
    else
    {
        if (left3->mClass == right3->mClass && left3->mArray == right3->mArray && left3->mNumGenericsTypes == right3->mNumGenericsTypes)
        {
            ...
            {
                if (type_identify_with_class_name(right3->mGenericsTypes[i],
                                                  "Null"))
                {
                    return 0;
                }
                if (!substitution_posibility(left3->mGenericsTypes[i],
                                             right3->mGenericsTypes[i],
                                             left_generics_types,
                                             right_generics_types,
                                             left_method_generics,
                                             right_method_generics))
...
int substitution_posibility_with_class_name(sNodeType * left,
                                            char * right_class_name)
{
    return substitution_posibility(left,
                                   create_node_type_with_class_name(right_class_name),
                                   (void *) 0,
                                   (void *) 0,
                                   (void *) 0,
                                   (void *) 0);
...
int no_cast_types_for_binary_operator(sNodeType * left_type,
                                      sNodeType * right_type)
{
    return type_identify_with_class_name(left_type,
                                         "pointer") && type_identify_with_class_name(right_type,
                                                                                     "ulong");
}
int operand_posibility(sNodeType * left,
                       sNodeType * right,
                       char * op_string)
{
    if (type_identify_with_class_name(left,
                                      "pointer") && (strcmp(op_string,
                                                            "+") == 0 || strcmp(op_string,
                                                                                "-") == 0))
    {
        ...
        {
            return type_identify_with_class_name(right,
                                                 "ulong") || type_identify_with_class_name(right,
                                                                                           "pointer");
        }
    }
    else
...
int operand_posibility_with_class_name(sNodeType * left,
                                       ...
                                       char * op_string)
{
    return operand_posibility(left,
                              create_node_type_with_class_name(right_class_name),
                              op_string);
}
int type_identify(sNodeType * left, sNodeType * right)
...
{
    sCLClass * klass2;
    klass2 = get_class_with_load_on_compile_time(class_name);
    ;
    return klass == klass2;
}
static void solve_Self_types_for_node_type(sNodeType * node_type,
                                           sNodeType * * result,
                                           sNodeType * generics_type)
{
    if (type_identify_with_class_name(node_type, "Self"))
    {
        ...
        *result = alloc_node_type();
        ...
        {
            (void) solve_generics_types_for_node_type(node_type->mGenericsTypes[j],
                                                      &(*result)->mGenericsTypes[j],
                                                      generics_type,
                                                      1,
                                                      0);
        }
                                       ...
                                       int solve_method_generics)
{
    ...
    if (solve_Self)
    {
        solve_Self_types_for_node_type(node_type,
                                       &node_type2,
                                       generics_type);
    }
    else
    ...
    {
        *result = clone_node_type(node_type2);
    }
    else if (generics_type && generics_type->mNumGenericsTypes > 0)
    {
        ...
        for (i = 0; i < 32; i++)
        {
            if (generics_param_class_num == i)
            {
                ...
                {
                    *result = clone_node_type(generics_type->mGenericsTypes[i]);
        ...
        *result = alloc_node_type();
        ...
        {
            (void) solve_generics_types_for_node_type(node_type2->mGenericsTypes[j],
                                                      &(*result)->mGenericsTypes[j],
                                                      generics_type,
                                                      1,
                                                      solve_method_generics);
        }
        if (node_type2->mBlockType)
        {
            sNodeBlockType * block_type;
            sNodeBlockType * result_block_type;
            block_type = node_type2->mBlockType;
            result_block_type = alloc_node_block_type();
            result_block_type->mNumParams = block_type->mNumParams;
            for (j = 0; j < block_type->mNumParams; j++)
            {
                (void) solve_generics_types_for_node_type(block_type->mParams[j],
                                                          &result_block_type->mParams[j],
                                                          generics_type,
                                                          1,
                                                          solve_method_generics);
            }
            (void) solve_generics_types_for_node_type(block_type->mResultType,
                                                      &result_block_type->mResultType,
                                                      generics_type,
                                                      1,
                                                      solve_method_generics);
...
sNodeType * create_generics_types_from_generics_params(sCLClass * klass)
{
    ...
    for (i = 0; i < klass->mNumGenerics; i++)
    {
        int offset;
        sCLClass * interface;
        offset = klass->mGenericsParamTypeOffsets[i];
        interface = get_class_with_load_on_compile_time((char *) ((&klass->mConst)->mConst + offset));
        ;
...
void solve_generics_for_variable_to_class(sCLClass * klass,
                                          ...
                                          sParserInfo * info)
{
    sCLClass * generics_class;
    int generics_param_number;
    generics_class = info->klass;
    generics_param_number = klass->mGenericsParamClassNum;
    *result = klass;
                                                 ...
                                                 struct sParserInfoStruct * info)
{
    ...
    int generics_param_number;
    generics_class = info->klass;
    generics_param_number = klass->mMethodGenericsParamClassNum;
    *result = klass;
    if (generics_param_number != -1)
    {
                                 ...
                                 sParserInfo * info)
{
    ...
    *generics_type2 = alloc_node_type();
    ...
    {
        int generics_param_number;
        int method_generics_param_number;
        generics_param_number = klass->mGenericsParamClassNum;
        if (generics_param_number != -1)
        {
        ...
        method_generics_param_number = klass->mMethodGenericsParamClassNum;
    ...
    {
        solve_generics_for_variable(generics_type->mGenericsTypes[i],
                                    &(*generics_type2)->mGenericsTypes[i],
                                    info);
    }
...
int is_exception_type(sNodeType * exception_type)
{
    return substitution_posibility_with_class_name(exception_type,
                                                   "Exception");
}
void print_node_type(sNodeType * node_type)
{
    if (node_type->mNumGenericsTypes == 0)
    {
        ...
        else if (node_type->mClass == (void *) 0)
        {
                      ...
                      sNodeType * right_type)
{
    if (left_type->mNumGenericsTypes == 0 && right_type->mNumGenericsTypes == 0)
    {
        sCLClass * left_class;
        sCLClass * right_class;
        left_class = left_type->mClass;
        right_class = right_type->mClass;
...
int unboxig_posibility(sCLClass * klass)
{
    return !(klass->mFlags & 0x1) && klass->mUnboxingClass != (void *) 0;
}
void boxing_to_lapper_class(sNodeType * * type_,
                            struct sCompileInfoStruct * info)
{
    ...
    else if (klass->mFlags & 0x1)
    {
        if (klass->mBoxingClass != (void *) 0)
        {
            sNodeType * boxing_type;
            boxing_type = create_node_type_with_class_pointer(klass->mBoxingClass);
            cast_right_type_to_left_type(boxing_type, type_, info);
        }
        else
        {
            parser_err_msg(info->pinfo,
                           "primitive class without boxing %s",
...
int unboxing_to_primitive_type(sNodeType * * left_type,
                               struct sCompileInfoStruct * info)
{
    sCLClass * primitive_class;
    sNodeType * primitive_type;
    primitive_class = (*left_type)->mClass->mUnboxingClass;
    primitive_type = create_node_type_with_class_pointer(primitive_class);
...
void make_boxing_type(sNodeType * type, sNodeType * * result)
{
    ...
    *result = alloc_node_type();
    ...
    if (type->mArray)
    {
        ...
        node_type = create_node_type_with_class_pointer(klass);
        make_boxing_type(node_type, &(*result)->mGenericsTypes[0]);
    }
    else if (klass->mFlags & 0x1 && klass->mBoxingClass)
    {
        int i;
        ...
        {
            make_boxing_type(type->mGenericsTypes[i],
                             &(*result)->mGenericsTypes[i]);
        }
    }
    else
    {
