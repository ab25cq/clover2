                  ...
                  int no_output)
{
    if (!no_output)
    {
        ...
        data2 = xcalloc(1, size);
        memcpy(data2, data, size);
        if (self->mSize <= self->mLen + size + 1)
        {
            int new_size;
            char * new_constant;
            new_size = (self->mLen + size + 1) * 2;
            new_constant = xcalloc(1, new_size);
                                ...
                                int no_output)
{
    ...
    int result;
    len = strlen(str);
    result = sConst_append(constant, str, len + 1, no_output);
    return result;
}
int append_wstr_to_constant_pool(sConst * constant,
                                 char * str,
                                 int no_output)
{
    ...
    int result;
    len = strlen(str);
    wcs = xmalloc(sizeof(wchar_t) * (len + 1));
    (void) mbstowcs(wcs, str, len + 1);
    result = sConst_append(constant,
                           wcs,
...
{
    int offset;
    offset = append_str_to_constant_pool(constant, str, no_output);
...
{
    int offset;
    offset = sConst_append(constant, buf, size, no_output);
    append_int_value_to_code(code, offset, no_output);
}
