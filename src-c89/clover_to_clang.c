...
wchar_t * string_object_to_wchar_array(CLObject string_object)
{
    sCLObject * string_object_data;
    CLObject wstr_array;
    sCLObject * wstr_array_data;
    int len;
    wchar_t * wstr;
    int i;
    string_object_data = (sCLObject *) get_object_pointer(string_object);
    wstr_array = string_object_data->mFields[0].mObjectValue;
    wstr_array_data = (sCLObject *) get_object_pointer(wstr_array);
    len = wstr_array_data->mArrayNum;
    wstr = xcalloc(1, sizeof(wchar_t) * (len + 1));
    for (i = 0; i < len; i++)
    {
...
char * string_object_to_char_array(CLObject string_object)
{
    ...
    int size;
    char * result;
    wstr = string_object_to_wchar_array(string_object);
    len = wcslen(wstr);
    size = sizeof(char) * 16 * (len + 1);
...
CLVALUE * get_element_from_Array(CLObject array, int index)
{
    ...
    CLObject items;
    array_data = (sCLObject *) get_object_pointer(array);
    items = array_data->mFields[0].mObjectValue;
...
int get_element_number_from_Array(CLObject array)
{
    sCLObject * array_data;
    CLObject items;
    sCLObject * items_data;
    array_data = (sCLObject *) get_object_pointer(array);
    items = array_data->mFields[0].mObjectValue;
    items_data = (sCLObject *) get_object_pointer(items);
...
{
    sCLObject * obj_data;
    obj_data = (sCLObject *) get_object_pointer(buffer);
...
CLObject * list_to_array(CLObject list, int * num_elements)
{
    ...
    int n;
    CLObject it;
    object_data = (sCLObject *) get_object_pointer(list);
    *num_elements = object_data->mFields[2].mIntValue;
    result = xcalloc(1, sizeof(CLObject) * *num_elements);
    n = 0;
    it = object_data->mFields[0].mObjectValue;
    while (it)
    {
        sCLObject * object_data;
        object_data = (sCLObject *) get_object_pointer(it);
        result[n] = object_data->mFields[0].mObjectValue;
        n++;
        it = object_data->mFields[1].mObjectValue;
    }
    return result;
}
void clover_termios_to_c_termios(CLObject terminfo_object,
                                 struct termios * terminfo_value)
{
    ...
    int i;
    object_data = (sCLObject *) get_object_pointer(terminfo_object);
    ...
    array = object_data->mFields[4].mObjectValue;
    object_data2 = (sCLObject *) get_object_pointer(array);
    for (i = 0; i < 32; i++)
    {
