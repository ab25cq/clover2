...
int CLParser_initialize(CLVALUE * * stack_ptr,
                        ...
                        sVMInfo * info);
struct sHandle_ {
    int mOffset; int mNextFreeHandle;
};
...
void heap_init(int heap_size, int size_hadles)
{
    gCLHeap.mMem = xcalloc(1, heap_size);
    gCLHeap.mMemSize = heap_size;
    gCLHeap.mMemLen = 0;
    gCLHeap.mMemB = xcalloc(1, heap_size);
    gCLHeap.mCurrentMem = gCLHeap.mMem;
    gCLHeap.mSleepMem = gCLHeap.mMemB;
    gCLHeap.mHandles = xcalloc(1, sizeof(sHandle) * size_hadles);
...
{
    int result;
    result = obj >= 1234 && obj < 1234 + gCLHeap.mNumHandles;
    return result;
}
void mark_object(CLObject obj, unsigned char * mark_flg)
{
    ...
    {
        if (mark_flg[obj - 1234] == 0)
        {
            sCLHeapMem * object;
            sCLClass * klass;
            int array_num;
            mark_flg[obj - 1234] = 1;
            object = get_object_pointer(obj);
            klass = object->mClass;
            array_num = object->mArrayNum;
            if (array_num == -1)
            {
                if (klass && !(klass->mFlags & 0x1))
                {
...
static void compaction(unsigned char * mark_flg)
{
    ...
    for (i = 0; i < gCLHeap.mNumHandles; i++)
    {
        if (gCLHeap.mHandles[i].mOffset != -1)
        {
            ...
            klass = ((sCLHeapMem *) data)->mClass;
            array_num = ((sCLHeapMem *) data)->mArrayNum;
            obj = i + 1234;
            if (!mark_flg[i])
            {
                if (klass && !(klass->mFlags & 0x20) && array_num == -1)
                {
    ...
    for (i = 0; i < gCLHeap.mNumHandles; i++)
    {
        if (gCLHeap.mHandles[i].mOffset != -1)
        {
            ...
            else
            {
                ...
                void * src;
                void * dst;
                obj_size = get_object_pointer(obj)->mSize;
...
static void delete_all_object()
{
    ...
    mark_flg = xcalloc(1, gCLHeap.mNumHandles);
    compaction(mark_flg);
    xfree(mark_flg);
}
static void show()
{
    ...
    for (i = 0; i < gCLHeap.mNumHandles; i++)
    {
        if (gCLHeap.mHandles[i].mOffset != -1)
        {
            ...
            klass = ((sCLHeapMem *) data)->mClass;
            obj = i + 1234;
            object_data = (sCLObject *) get_object_pointer(obj);
            printf("obj %d size %d array_num %d\n",
                   obj,
                   object_data->mSize,
                   object_data->mArrayNum);
        }
    }
}
static void gc()
...
CLObject alloc_heap_mem(int size, sCLClass * klass, int array_num)
{
    int handle;
    CLObject obj;
    sCLHeapMem * object_ptr;
    if (gCLHeap.mMemLen + size >= gCLHeap.mMemSize)
    {
        gc();
        if (gCLHeap.mMemLen + size >= gCLHeap.mMemSize)
        {
            int current_is_mem_a;
            int new_heap_size;
            current_is_mem_a = gCLHeap.mMem == gCLHeap.mCurrentMem;
            new_heap_size = (gCLHeap.mMemLen + size + 1) * 10;
            gCLHeap.mMem = xrealloc(gCLHeap.mMem, new_heap_size);
            memset(gCLHeap.mMem + gCLHeap.mMemSize,
                   0,
                   new_heap_size - gCLHeap.mMemSize);
            gCLHeap.mMemB = xrealloc(gCLHeap.mMemB, new_heap_size);
            memset(gCLHeap.mMemB + gCLHeap.mMemSize,
                   0,
                   new_heap_size - gCLHeap.mMemSize);
    ...
    else
    {
        ...
        {
            const int new_offset_size;
            new_offset_size = (gCLHeap.mSizeHandles + 1) * 10;
            gCLHeap.mHandles = xrealloc(gCLHeap.mHandles,
                                        sizeof(sHandle) * new_offset_size);
            memset(gCLHeap.mHandles + gCLHeap.mSizeHandles,
                   0,
                   sizeof(sHandle) * (new_offset_size - gCLHeap.mSizeHandles));
    ...
    object_ptr = get_object_pointer(obj);
    object_ptr->mSize = size;
    object_ptr->mClass = klass;
    object_ptr->mType = (void *) 0;
